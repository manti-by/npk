...
class mosPoll extends mosDBTable {
	...
	*/
	...
	function check() {
		...
		if (trim( $this->title ) == '') {
			$this->_error = 'Голосование должно иметь заголовок.';
			return false;
		}
		// check for valid lag
		$this->lag = intval( $this->lag );
		if ($this->lag == 0) {
			$this->_error = 'Голосование должно иметь ненулевое время ожидания.';
			return false;
		}
		...
		if ( $xid && $xid != intval( $this->id ) ) {
			$this->_error = 'Уже есть модуль с таким именем, попробуйте еще раз.';
			return false;
		}
