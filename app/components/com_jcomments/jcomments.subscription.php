 ...
 *
 */
class JCommentsSubscriptionsDB extends mosDBTable
{
	/** @var int Primary key */
	var $id = null;
	/** @var int */
	...
	*/
	function JCommentsSubscriptionsDB( &$db ) {
		$this->mosDBTable('#__jcomments_subscriptions', 'id', $db);
	}
}
...
class JCommentsSubscriptionManager
{
	...
	function getHash($object_id, $object_group, $userid, $email = '', $lang = '')
	{
		...
		if (JCommentsMultilingual::isEnabled() && $lang == '') {
	...
	function subscribe($object_id, $object_group, $userid, $email = '', $name = '', $lang = '')
	{
		...
		if ($this->isSubscribed($object_id, $object_group, $userid, $email, $lang) == 0) {
	...
	function unsubscribe($object_id, $object_group, $userid)
	{
		if ($userid != 0) {
			...
			$query = "DELETE"
					...
					. "\n AND object_group = '" . mysql_escape_string($object_group) . "'"
