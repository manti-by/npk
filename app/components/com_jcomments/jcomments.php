...
switch(trim($jc_task)) {
	...
	case 'go2object':
		...
		break;
	default:
		if ($jc_option == 'com_jcomments' && $jc_ajax == '') {
...
class JComments
{
	...
	function show( $object_id, $object_group = 'com_content', $object_title = '' )
	{
		...
		if (!defined('JCOMMENTS_CSS')) {
			if ($mainframe->isAdmin()) {
				...
				$link = $tmpl->getBaseURI() . '/style.css';
	...
	function getCommentsList( $object_id, $object_group = 'com_content', $page = 0 )
	{
		...
		if (count($rows)) {
			...
			$tmpl->addVar( 'tpl_list', 'comments-rss', $config->getInt('enable_rss'));
			$tmpl->addVar( 'tpl_list', 'comments-can-subscribe', intval($my->id && $acl->check('enable_subscribe')));
			...
			foreach ($rows as $row) {
				...
				if (!$acl->canModerate($row)) {
					...
					$tmpl->addVar('tpl_comment', 'button-ip', $acl->canViewIP());
				}
				...
				$tmpl->addVar('tpl_comment', 'button-quote', $acl->canQuote());
				$tmpl->addVar('tpl_comment', 'button-reply', $acl->canReply());
	...
	function getCommentsTree( $object_id, $object_group = 'com_content' )
	{
		...
		if (count($rows)){
			$tmpl->addVar('tpl_tree', 'comments-rss', $config->getInt('enable_rss'));
			$tmpl->addVar('tpl_tree', 'comments-can-subscribe', intval($my->id && $acl->check('enable_subscribe')));
	...
	function sendNotification( &$comment, $isNew = true)
	{
		...
		if ($config->get('notification_email') != '') {
			...
			if (JCOMMENTS_JVERSION == '1.5') {
				...
				$object->object_link = $url . $object->object_link;
			}
			...
			$tmpl = & JCommentsFactory::getTemplate($comment->object_id, $comment->object_group);
			$tmpl->load('tpl_email');
			$tmpl->addVar('tpl_email', 'notification-type', 'admin');
			$tmpl->addVar('tpl_email', 'comment-isnew', ($isNew) ? 1 : 0);
			$tmpl->addVar('tpl_email', 'comment-object_title', $object->object_title);
			$tmpl->addVar('tpl_email', 'comment-object_link', $object->object_link);
			$tmpl->addObject('tpl_email', 'comment', $comment);

			// render template
			$tmpl->renderTemplate('tpl_email');

			// load variables from rendered template
			$subject = $tmpl->getVar('tpl_email', 'subject');
			$message = $tmpl->getVar('tpl_email', 'message');
	...
	function prepareComment( &$comment )
	{
		...
		if (empty($comment->avatar)) {
			$comment->avatar = '<img src="http://www.gravatar.com/avatar.php?gravatar_id='. md5($comment->email) .'&amp;default=' . urlencode($mainframe->getCfg('live_site') . '/components/com_jcomments/images/no_avatar.png') . '" alt="" />';
		}
