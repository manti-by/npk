...
class JCommentsPluginHelper
{
	...
	function getParams($pluginName, $type = 'content')
	{
		...
		if (JCOMMENTS_JVERSION == '1.5') {
			...
			global $_MAMBOTS;
			if (!isset($_MAMBOTS->_search_mambot_params[$pluginName])) {
				$mambot = null;
				$dbo = & JCommentsFactory::getDBO();
				$dbo->setQuery("SELECT params FROM #__mambots WHERE element = '$pluginName' AND folder = '$type'");
				$dbo->loadObject($mambot);
				$_MAMBOTS->_search_mambot_params[$pluginName] = $mambot;
			}

			$plugin = $_MAMBOTS->_search_mambot_params[$pluginName];
			$pluginParams = new mosParameters($plugin->params);
		}
		return $pluginParams;
	}
}
