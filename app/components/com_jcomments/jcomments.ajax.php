		...
		if ($acl->canComment()) {
			...
			if (($acl->check('floodprotection') == 1) && (JCommentsSecurity::checkFlood($userIP))) {
				...
				$query = "SELECT COUNT(*) "
						...
						. "\n  AND ip = '" . $comment->ip . "'"
						. "\n  AND name = '" . mysql_escape_string($comment->name) . "'"
						. "\n  AND userid = '" . $comment->userid . "'"
						. "\n  AND object_id = " . $comment->object_id
						. "\n  AND parent = " . $comment->parent
						. "\n  AND object_group = '" . mysql_escape_string($comment->object_group) . "'"
		...
		if ($comment->load($id)) {
			...
			if ($acl->isLocked($comment)) {
				$response->addAlert(JText::_('ERROR_BEING_EDITTED'));
					...
					$response->addScript("jcomments.showEdit(" . $comment->id . ", '$name', '$email', '$homepage', '$text');");
				} else {
