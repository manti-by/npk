	...
	*/
    function initSessionAdmin($option, $task)
    {
        ...
        if ($session_id != session_id()) {
            ...
            echo "<script>document.location.href='index.php?mosmsg=Неверная сессия'</script>\n";
            exit();
        }
        ...
        if ($session_id == md5($my->id . $my->username . $my->usertype . $logintime)) {
            ...
            if ($task != 'save' && $task != 'apply') {
                ...
                if ($count == 0) {
                    ...
                    echo "<script>document.location.href='index.php?mosmsg=Сессия админа просрочена'</script>\n";
                    exit();
                } else {
            ...
            if (ini_get('session.auto_start') || !ini_get('session.use_cookies')) {
                echo "<script>document.location.href='index.php?mosmsg=Требуется войти заново. Если session.auto_start в php.ini в состоянии -on- или session.use_cookies в -off-, возможно требуется корректировка этих значений.'</script>\n";
            } else {
                echo "<script>document.location.href='index.php?mosmsg=Требуется войти заново'</script>\n";
            }
            exit();
        } else {
            // session id does not correspond to required session format
...
class mosHTML
{
    ...
    {
        $writeable = '<b><font color="green">Доступен на запись</font></b>';
        $unwriteable = '<b><font color="red">Недоступен на запись</font></b>';
...
class mosCategory extends mosDBTable
{
    ...
    function check()
    {
        ...
        if (trim($this->title) == '') {
            $this->_error = "Категория должна иметь заголовок.";
            return false;
        }
        if (trim($this->name) == '') {
            $this->_error = "Категория должна иметь имя.";
            return false;
        }
        ...
        if ($xid && $xid != intval($this->id)) {
            $this->_error = "Категория с таким именем уже существует, введите другое имя.";
            return false;
        }
        return true;
    }
...
class mosSection extends mosDBTable
{
    ...
    function check()
    {
        // check for valid name
        if (trim($this->title) == '') {
            $this->_error = "Раздел должен иметь заголовок.";
            return false;
        }
        if (trim($this->name) == '') {
            $this->_error = "Раздел должен иметь имя.";
            return false;
        }
        $ignoreList = array('description');
        $this->filter($ignoreList);
        ...
        $xid = intval($this->_db->loadResult());
        if ($xid && $xid != intval($this->id)) {
            $this->_error = "Раздел с таким именем уже существует, введите другое имя.";
            return false;
        }
        return true;
    }
...
class mosUser extends mosDBTable
{
    ...
    function store($updateNulls = false)
    {
        ...
        if (!$ret) {
            $this->_error = strtolower(get_class($this)) . "::сохранение сорвано <br />" . $this->_db->getErrorMsg();
            return false;
        } else {
            return true;
        }
...
class mosModule extends mosDBTable
{
    ...
    function check()
    {
        // check for valid name
        if (trim($this->title) == '') {
            $this->_error = "Модуль должен иметь заголовок";
            return false;
        }
...
class mosSession extends mosDBTable
{
    ...
    function insert()
    {
        ...
        if (!$ret) {
            $this->_error = strtolower(get_class($this)) . "::сохранение сорвано <br />" . $this->_db->stderr();
            return false;
        } else {
            return true;
        }
 ...
 */
function mosWarning($warning, $title = 'Joomla Lavra! Предупреждение')
{
    global $mosConfig_live_site;
    ...
    $tip .= '<img src="' . $mosConfig_live_site . '/includes/js/ThemeOffice/warning.png" border="0"  alt="Внимание"/></a>';
...
class mosAdminMenus
{
    ...
    function Target(&$row)
    {
        $click[] = mosHTML::makeOption('0', 'То же окно с кнопками навигации');
        $click[] = mosHTML::makeOption('1', 'Новое окно с кнопками навигации');
        $click[] = mosHTML::makeOption('2', 'Новое окно без кнопкок навигации');
    ...
    function MenuLinks(&$lookup, $all = NULL, $none = NULL, $unassigned = 1)
    {
        ...
        if ($none) {
            // prepare an array with 'all' as the first item
            $mitems[] = mosHTML::makeOption(-999, 'Нет');
            // adds space, in select box which is not saved
            $mitems[] = mosHTML::makeOption(-999, '----');
        }
        if ($unassigned) {
            // prepare an array with 'all' as the first item
            $mitems[] = mosHTML::makeOption(99999999, 'Не назначено');
            // adds space, in select box which is not saved
            $mitems[] = mosHTML::makeOption(-999, '----');
        }
    ...
    function Images($name, &$active, $javascript = NULL, $directory = NULL)
    {
        ...
        $images = array(mosHTML::makeOption('', '- Выбор картинки -'));
    ...
    function ComponentCategory($name, $section, $active = NULL, $javascript = NULL, $order = 'ordering', $size = 1, $sel_cat = 1)
    {
        ...
        if (count($categories) < 1) {
            mosRedirect('index2.php?option=com_categories&section=' . $section, 'Сначала создайте категорию');
        }
    ...
    function ReadImagesX(&$folders, &$images)
    {
        ...
        if ($folders[0]->value != '*0*') {
            ...
            $folders = array();
            $folders[] = mosHTML::makeOption('Нет');
        }
    }

    function GetImageFolders(&$temps, $path)
    {
        if ($temps[0]->value != 'Нет') {
            ...
            $folders[] = mosHTML::makeOption('Ничего не выбрано');
        }
...
class mosCommonHTML
{
    ...
    function ContentLegend()
    {
        ...
        <table cellspacing="0" cellpadding="4" border="0" align="center">
            <tr align="center">
                <td>
                    <img src="images/publish_y.png" width="12" height="12" border="0" alt="Ожидает"/>
                </td>
                <td>
                    Опубликовано и <u>Ожидает</u> |
                </td>
                <td>
                    <img src="images/publish_g.png" width="12" height="12" border="0" alt="Активно"/>
                </td>
                <td>
                    Опубликовано и <u>Активно</u> |
                </td>
                <td>
                    <img src="images/publish_r.png" width="12" height="12" border="0" alt="Просрочено"/>
                </td>
                <td>
                    Опубликовано, но <u>Просрочено</u> |
                </td>
                <td>
                    <img src="images/publish_x.png" width="12" height="12" border="0" alt="Закончено"/>
                </td>
                <td>
                    Не опубликовано
                </td>
            ...
            <tr>
                <td colspan="8" align="center">
                    Щелкните по иконке для переключения состояния.
                </td>
            </tr>
        </table>
        <?php
    ...
    function menuLinksContent(&$menus)
    {
        ...
        foreach ($menus as $menu) {
            ...
            <tr>
                ...
                <td>
                    <a href="javascript:go2( 'go2menu', '<?php echo $menu->menutype; ?>' );" title="Идти к меню">
            ...
            <tr>
                ...
                <td>
                    ...
                    switch ($menu->published) {
                        case -2:
                            echo '<font color="red">В корзине</font>';
                            break;
                        case 0:
                            echo 'Неопубликовано';
                            break;
	...
	*/
    ...
    function PublishedProcessing(&$row, $i)
    {
        ...
        $action = $row->published ? 'Скрыть объект' : 'Опубликовать объект';
