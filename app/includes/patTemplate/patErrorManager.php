...
class patErrorManager
{
    ...
    function setErrorHandling( $level, $mode, $options = null )
    {
		...
		if( !is_callable( array( 'patErrorManager', $function ) ) )
		{
			return patErrorManager::raiseError( E_ERROR,
												...
												'Ошибка обработки режима, режим неизвестен',
												'Режим: ' .  $mode . ' не поддерживаеться.'
												);
		}
			...
			if( $mode == 'callback' )
			{
				if( !is_array( $options ) )
				{
					return patErrorManager::raiseError( E_ERROR,
														...
														'Опция для обратного вызова недопустима'
														);
				}
				...
				if( !is_callable( $options ) )
				{
					...
					return patErrorManager::raiseError(	E_ERROR,
														...
														'Функция не может быть вызвана',
														'Функция:' . $tmp[1]  . ' scope ' . $tmp[0] . '.'
														);
				}
			}
	...
	*/
	function translateErrorLevel( $level )
	{
		...
		return	'Ошибка на неизвестном уровне';
	}
