...
class patTemplate
{
		...
		switch( strtolower( $type ) )
		{
			...
			default:
				return	patErrorManager::raiseWarning(
														...
														"Неизвестный тип '$type'. Используйте 'html' или 'tex'."
													);
		}
		return true;
	}
	...
	*/
	function setAttributes( $template, $attributes )
	{
		...
		{
			return patErrorManager::raiseError( PATTEMPLATE_ERROR_EXPECTED_ARRAY, 'patTemplate::setAttributes: Ожидался массив следующих параметров, '.gettype( $attributes ).' получено' );
		}
	...
	*/
	function applyOutputFilter( $filter, $params = array(), $template = null )
	{
		...
		if (!$this->exists($template)) {
			return patErrorManager::raiseWarning(PATTEMPLATE_WARNING_NO_TEMPLATE, 'Выбранный шаблон не существует');
		}
	...
	*/
	...
	{
		if ((string)$input === '') {
			return patErrorManager::raiseError(PATTEMPLATE_ERROR_NO_INPUT, 'Не задан входной поток для чтения.');
		}
	...
	function &loadModule( $moduleType, $moduleName, $params = array(), $new = false )
	{
		...
		if( !class_exists( 'patTemplate_Module' ) )
		{
			...
			if( !@include_once $file )
				return	patErrorManager::raiseError( PATTEMPLATE_ERROR_BASECLASS_NOT_FOUND, 'Не могу загрузить базовый класс модуля.' );
		}
		...
		if( !class_exists( $baseClass ) )
		{
			...
			if( !@include_once $baseFile )
				return	patErrorManager::raiseError( PATTEMPLATE_ERROR_BASECLASS_NOT_FOUND, "Не могу загрузить базовый класс для $moduleType ($baseFile)." );
		}
		...
		if( !class_exists( $moduleClass ) )
		{
			...
			if( !$found ) {
				return	patErrorManager::raiseError( PATTEMPLATE_ERROR_MODULE_NOT_FOUND, "Не могу загрузить модуль $moduleClass ($moduleFile)." );
			}
		}

		if( !class_exists( $moduleClass ) )
		{
			return	patErrorManager::raiseError( PATTEMPLATE_ERROR_MODULE_NOT_FOUND, "Файл модуля $moduleFile не содержит класс $moduleClass." );
		}
	...
	function _parseDependencies($template)
	{
		...
		for ($i = 0; $i < $countDep; $i++) {
			...
			if ($depTemplate == $template) {
				return patErrorManager::raiseError(PATTEMPLATE_ERROR_RECURSION, 'У вас ошибка в шаблоне "' . $template . '", рекурсивный вызов.');
			}
		...
		if( isset( $this->_templates[$template]['attributes']['varscope'] ) )
		{
			...
			foreach ($this->_templates[$template]['attributes']['varscope'] as $scopeTemplate) {
				if ($this->exists($scopeTemplate)) {
					...
					patErrorManager::raiseWarning(PATTEMPLATE_WARNING_NO_TEMPLATE, 'Шаблон \''.$scopeTemplate.'\' не существует, ссылка в varscope атрибуте шаблона \''.$template.'\'');
				}
			}
		} else {
			$vars	=	array();
