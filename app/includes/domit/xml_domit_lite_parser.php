...
class DOMIT_Lite_Node {
	...
	*/
	function DOMIT_Lite_Node() {
		DOMIT_DOMException::raiseException(DOMIT_ABSTRACT_CLASS_INSTANTIATION_ERR,
			 'Не могу инициализировать абстрактный класс DOMIT_Lite_Node');
	} //DOMIT_Lite_Node
	...
	*/
	function &appendChild(&$child) {
		DOMIT_DOMException::raiseException(DOMIT_HIERARCHY_REQUEST_ERR,
			('Метод appendChild не может быть вызван классом ' . get_class($this)));
	} //appendChild
	...
	*/
	function &insertBefore(&$newChild, &$refChild) {
		DOMIT_DOMException::raiseException(DOMIT_HIERARCHY_REQUEST_ERR,
			('Метод insertBefore не может быть вызван классом ' . get_class($this)));
	} //insertBefore
	...
	*/
	function &replaceChild(&$newChild, &$oldChild) {
		DOMIT_DOMException::raiseException(DOMIT_HIERARCHY_REQUEST_ERR,
			('Метод replaceChild не может быть вызван классом ' . get_class($this)));
	} //replaceChild
	...
	*/
	function &removeChild(&$oldChild) {
		DOMIT_DOMException::raiseException(DOMIT_HIERARCHY_REQUEST_ERR,
			('Метод removeChild не может быть вызван классом ' . get_class($this)));
	} //removeChild
	...
	*/
	function &cloneNode($deep = false) {
		DOMIT_DOMException::raiseException(DOMIT_ABSTRACT_METHOD_INVOCATION_ERR,
			 'Не могу вызвать абстрактный метод DOMIT_Lite_Node->cloneNode($deep). Этот метод должен быть обеспечен в вашем подклассе.');
	} //cloneNode
	...
	*/
	function &getElementsByPath($pattern, $nodeIndex = 0) {
		 DOMIT_DOMException::raiseException(DOMIT_HIERARCHY_REQUEST_ERR,
			('Метод getElementsByPath не может быть вызван классом ' . get_class($this)));
	} //getElementsByPath
	...
	*/
	function toArray() {
		DOMIT_DOMException::raiseException(DOMIT_HIERARCHY_REQUEST_ERR,
			('Метод toArray не может быть вызван классом ' . get_class($this)));
	} //toArray
...
class DOMIT_Lite_ChildNodes_Interface extends DOMIT_Lite_Node {
	...
	*/
	function DOMIT_Lite_ChildNodes_Interface() {
		DOMIT_DOMException::raiseException(DOMIT_ABSTRACT_CLASS_INSTANTIATION_ERR,
			 'Не могу инициализировать абстрактный класс DOMIT_Lite_ChildNodes_Interface');
	...
	function &insertBefore(&$newChild, &$refChild) {
		if (($refChild->nodeType == DOMIT_DOCUMENT_NODE)  ||
			...
			DOMIT_DOMException::raiseException(DOMIT_NOT_FOUND_ERR,
				 'Потомок по ссылке не описан в списке потомков.');
		}
	...
	*/
	function &removeChild(&$oldChild) {
		...
		DOMIT_DOMException::raiseException(DOMIT_NOT_FOUND_ERR,
				('Узел назначения removeChild не найден.'));
	} //removeChild
	...
	*/
	function &setDocumentElement(&$node) {
		if ($node->nodeType == DOMIT_ELEMENT_NODE) {
			...
			DOMIT_DOMException::raiseException(DOMIT_HIERARCHY_REQUEST_ERR,
				('Не могу добавить узел типа ' . get_class($node) . ' как элемент документа.'));
		}
	...
	function &appendChild(&$node) {
		if ($node->nodeType == DOMIT_ELEMENT_NODE) {
			if ($this->documentElement == null) {
				...
				DOMIT_DOMException::raiseException(DOMIT_HIERARCHY_REQUEST_ERR,
					('Нельзя иметь более одного коренного узла (documentElement) в DOMIT_Document.'));
			}
		}
		else {
			DOMIT_DOMException::raiseException(DOMIT_HIERARCHY_REQUEST_ERR,
				('Нельзя добавить узел типа ' . get_class($node) . ' в DOMIT_Document.'));
		}
	...
	function &replaceChild(&$newChild, &$oldChild) {
			if (($this->documentElement != null) && ($oldChild->uid == $this->documentElement->uid)) {
				if ($node->nodeType == DOMIT_ELEMENT_NODE) {
					...
					DOMIT_DOMException::raiseException(DOMIT_HIERARCHY_REQUEST_ERR,
						('Нельзя разместить Document Element с узлом класса ' . get_class($newChild)));
				}
			}
			else {
				if ($node->nodeType == DOMIT_ELEMENT_NODE) {
					...
					DOMIT_DOMException::raiseException(DOMIT_HIERARCHY_REQUEST_ERR,
						('Узел класса ' . get_class($newChild) . ' не может быть потомком DOMIT_Document.'));
				}
			}
	...
	*/
	function &insertBefore(&$newChild, &$refChild) {
		...
		if ($type == DOMIT_ELEMENT_NODE) {
			...
			DOMIT_DOMException::raiseException(DOMIT_HIERARCHY_REQUEST_ERR,
				('Нельзя вставить узел типа ' . get_class($newChild) . ' в DOMIT_Document.'));
		}
...
// Вот здесь эти австралийские попугаи опять закомментировали ошибочный парсер
// Наверно после того, как выяснилось, что кроме английского языка существуют еще и другие (надо же....)		
		...
		*/
		...
		if (version_compare(phpversion(), '5.0', '<=')) {
			...
			$parser = xml_parser_create(''); // ХРЕН АВСТАЛИЙСКИМ ПОПУГАЯМ!!!!!!!!!!!!!!!!!!!!!
		}
// И создадим парсер в iso-8859-1 кодировке , если версия РНР высокая...
// Мдя уж... с кириллицей в XML-файлах "фэйлом об тэйбл"
// т.е. версия РНР ни в коем случае не выше 5.0 - выше РНР не позволяет...
