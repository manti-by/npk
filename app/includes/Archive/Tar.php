...
class Archive_Tar extends PEAR
{
	...
	*/
	function Archive_Tar($p_tarname, $p_compress = null)
	{
		...
		if ($this->_compress) { // assert zlib or bz2 extension support
			...
			if (!extension_loaded($extname)) {
				die("Расширение '$extname' не найдено.\n".
					"Убедитесь, что ваша версия PHP имеет поддержку '$extname'.\n");
				return false;
			}
		}
	}
	...
	function createModify($p_filelist, $p_add_dir, $p_remove_dir='')
	{
		...
		if ($p_filelist != '') {
			...
			elseif (is_string($p_filelist))
				...
				$this->_cleanFile();
				$this->_error('Неверный список файлов');
				return false;
			}
	...
	*/
	function extractList($p_filelist, $p_path='', $p_remove_path='')
	{
		...
		elseif (is_string($p_filelist))
			...
			$this->_error('Неверный список строк');
			return false;
		}
	...
	*/
		...
		while ($i<$v_size) {
			...
			switch ($v_att_list[$i]) {
				...
				case ARCHIVE_TAR_ATT_SEPARATOR :
					...
					if (($i+1) >= $v_size) {
						$this->_error('Неверное количество параметров для атрибута ARCHIVE_TAR_ATT_SEPARATOR');
						return false;
					}
				...
				default :
					$this->_error('Неизвестный атрибут кода '.$v_att_list[$i].'');
					return false;
			}
	...
	function _openWrite()
	{
		if ($this->_compress_type == 'gz')
			...
			$this->_error('Неизвестный или отсутствующий тип компрессии ('.$this->_compress_type.')');

		if ($this->_file == 0) {
			$this->_error('Не могу открыть на запись \''.$this->_tarname.'\'');
			return false;
		}
	...
	function _openRead()
	{
		if (strtolower(substr($this->_tarname, 0, 7)) == 'http://') {
		  ...
		  if ($this->_temp_tarname == '') {
			  ...
			  if (!$v_file_from = @fopen($this->_tarname, 'rb')) {
				$this->_error('Не могу открыть на чтение \''.$this->_tarname.'\'');
				$this->_temp_tarname = '';
				return false;
			  }
			  if (!$v_file_to = @fopen($this->_temp_tarname, 'wb')) {
				$this->_error('Не могу открыть на запись \''.$this->_temp_tarname.'\'');
				$this->_temp_tarname = '';
				return false;
			  }
		...
		if ($this->_file == 0) {
			$this->_error('Не могу открыть на чтение \''.$v_filename.'\'');
			return false;
		}
	...
	function _openReadWrite()
	{
		...
		if ($this->_file == 0) {
			$this->_error('Не могу открыть на чтение-запись \''.$this->_tarname.'\'');
			return false;
		}
	...
	function _addList($p_list, $p_add_dir, $p_remove_dir)
	{
	  ...
	  if (!$this->_file) {
		  $this->_error('Неверный дескриптор файла');
		  return false;
	  }
	  ...
	  for ($j=0; ($j<count($p_list)) && ($v_result); $j++) {
		...
		if (!file_exists($v_filename)) {
			$this->_warning("Файл '$v_filename' не существует");
			continue;
		}
		...
		if (@is_dir($v_filename)) {
			if (!($p_hdir = opendir($v_filename))) {
				$this->_warning("Каталог '$v_filename' не может быть прочтен");
				continue;
			}
	...
	{
	  if (!$this->_file) {
		  $this->_error('Неверный дескритптор файла');
		  return false;
	  }

	  if ($p_filename == '') {
		  $this->_error('Неверное имя файла');
		  return false;
	  }
	  ...
	  if (is_file($p_filename)) {
		  if (($v_file = @fopen($p_filename, "rb")) == 0) {
			  $this->_warning("Не могу открыть файл '$p_filename' в режиме двоичного чтения");
			  return true;
		  }
	...
	{
	  if (!$this->_file) {
		  $this->_error('Неверный десткриптор файла');
		  return false;
	  }
	...
	function _readHeader($v_binary_data, &$v_header)
	{
		...
		if (strlen($v_binary_data) != 512) {
			$v_header['filename'] = '';
			$this->_error('Неверный размер блока : '.strlen($v_binary_data));
			return false;
		}
		...
		if ($v_header['checksum'] != $v_checksum) {
			...
			$this->_error('Неверная контрольная суммафайла "'.$v_data['filename'].'" : '.$v_checksum.' получено, '.$v_header['checksum'].' ожидалось');
			return false;
		}
	...
	*/
		...
		While (strlen($v_binary_data = $this->_readBlock()) != 0)
		{
		  ...
		  if ($v_header['filename'] == $p_filename) {
			  if ($v_header['typeflag'] == "5") {
				  $this->_error('Не мошу извлечь в строку компонент каталога {'.$v_header['filename'].'}');
				  return NULL;
			  } else {
				  $n = floor($v_header['size']/512);
				  for ($i=0; $i<$n; $i++) {
	...
	switch ($p_mode) {
	  ...
	  break;
	  default :
		$this->_error('Неверный режми извлечения ('.$p_mode.')');
		return false;
	}
	  ...
	  if (($v_extract_file) && (!$v_listing))
	  {
		...
		if (file_exists($v_header['filename'])) {
		  if ((@is_dir($v_header['filename'])) && ($v_header['typeflag'] == '')) {
			$this->_error('Файл '.$v_header['filename'].' уже существует в каталога');
			return false;
		  }
		  if ((is_file($v_header['filename'])) && ($v_header['typeflag'] == "5")) {
			$this->_error('Каталог '.$v_header['filename'].' уже существует как файл');
			return false;
		  }
		  if (!is_writeable($v_header['filename'])) {
			$this->_error('Файл '.$v_header['filename'].' уже существует и защищен от записи');
			return false;
		  }
		...
		elseif (($v_result = $this->_dirCheck(($v_header['typeflag'] == "5"?$v_header['filename']:dirname($v_header['filename'])))) != 1) {
			$this->_error('Не могу создать путь для '.$v_header['filename']);
			return false;
		}
		...
		if ($v_extract_file) {
		  if ($v_header['typeflag'] == "5") {
			if (!@file_exists($v_header['filename'])) {
				if (!@mkdir($v_header['filename'], 0777)) {
					$this->_error('не могу создать каталог для {'.$v_header['filename'].'}');
					return false;
				}
			}
		  } else {
			  if (($v_dest_file = @fopen($v_header['filename'], "wb")) == 0) {
				  $this->_error('Ошибка открытия {'.$v_header['filename'].'} в режиме двоичной записи');
				  return false;
			  } else {
				  $n = floor($v_header['size']/512);
				  for ($i=0; $i<$n; $i++) {
		  ...
		  if (filesize($v_header['filename']) != $v_header['size']) {
			  $this->_error('Извлеченный файл '.$v_header['filename'].' имеет неверный размер \''.filesize($v_filename).'\' ('.$v_header['size'].' ожидалось). Возможно архив поврежден.');
			  return false;
		  }
		  }
		} else {
	...
	function _openAppend()
	{
		...
		if ($this->_compress) {
			...
			if (!@rename($this->_tarname, $this->_tarname.".tmp")) {
				$this->_error('Ошибка при переименовании \''.$this->_tarname.'\' во временный файл \''.$this->_tarname.'.tmp\'');
				return false;
			}
			...
			if ($v_temp_tar == 0) {
				$this->_error('Не могуоткрыть файл \''.$this->_tarname.'.tmp\' в режиме двоичного чтения');
			...
			if (!@unlink($this->_tarname.".tmp")) {
				$this->_error('Ошибка при удалении временного файла \''.$this->_tarname.'.tmp\'');
			}
	...
	function _dirCheck($p_dir)
	{
		...
		if (!@mkdir($p_dir, 0777)) {
			$this->_error("Не могу создать каталог '$p_dir'");
			return false;
		}
