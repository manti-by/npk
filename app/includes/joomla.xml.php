	...
	function render( $name='params' ) {
		...
		if (is_object( $this->_xmlElem )) {
			...
			foreach ($element->childNodes as $param) {
				...
				$html[] = '<tr>';
				$result[0] = normal ($result[0]);
				$result[1] = normal ($result[1]);
				$result[2] = normal ($result[2]);
...
*/
	...
	*/
	function _form_mos_section( $name, $value, &$node, $control_name ) {
		...
		array_unshift( $options, mosHTML::makeOption( '0', '- Выбор раздела -', 'id', 'title' ) );
	...
	*/
	function _form_mos_category( $name, $value, &$node, $control_name ) {
		...
		$database->setQuery( $query );
		$options = $database->loadObjectList();
		array_unshift( $options, mosHTML::makeOption( '0', '- Выбор категории -', 'id', 'title' ) );
	...
	*/
	function _form_mos_menu( $name, $value, &$node, $control_name ) {
		...
		array_unshift( $options, mosHTML::makeOption( '', '- Выбор меню -' ) );
	...
	*/
	function _form_filelist( $name, $value, &$node, $control_name ) {
		...
		if ( !$node->getAttribute( 'hide_none' ) ) {
			array_unshift( $options, mosHTML::makeOption( '-1', '- '. 'Не использовать картинки' .' -' ) );
		}
		if ( !$node->getAttribute( 'hide_default' ) ) {
			array_unshift( $options, mosHTML::makeOption( '', '- '. 'Использовать картинку по умолчанию' .' -' ) );
		}
