...
class database {
	...
	*/
	function database( $host='localhost', $user, $pass, $db='', $table_prefix='', $goOffline=true ) {
		...
		// О! Опять! Уже два года убрать не могут! Свободу Австралийским попугаям!!!
		// Ну ни хрена со спецификацией MySQL не дружат кенгурушники...
                // SET NAMES оставлена в верхних версия мускуля только для совместимости, а в официальной документации ОТСУТСТВУЕТ!
	...
	*/
	function setQuery( $sql, $offset = 0, $limit = 0, $prefix='#__' ) {
		$this->_sql = $this->replacePrefix( pofigu($sql), $prefix );
	...
	*/
	function stderr( $showSQL = false ) {
		return "Вызов DB функции завершен с ошибкой номер: $this->_errorNum"
...
class mosDBTable {
	...
	*/
	function bind( $array, $ignore='' ) {
		if (!is_array( $array )) {
			$this->_error = strtolower(get_class( $this ))."::связывание неудачно.";
			return false;
		} else {
...
//echo 'D: ' . $this->_db->getQuery();
	...
	*/
	function updateOrder( $where='' ) {
		...
		if (!array_key_exists( 'ordering', get_class_vars( strtolower(get_class( $this )) ) )) {
			$this->_error = "ВНИМАНИЕ: ".strtolower(get_class( $this ))." не поддерживает изменение порядка.";
			return false;
		}
	...
	*/
	...
	function checkout( $user_id, $oid=null ) {
		if (!array_key_exists( 'checked_out', get_class_vars( strtolower(get_class( $this )) ) )) {
			$this->_error = "ВНИМАНИЕ: ".strtolower(get_class( $this ))." не поддерживает снятие блокировкм.";
			return false;
		}
		$k = $this->_tbl_key;
		if ($oid !== null) {
	...
	function checkin( $oid=null ) {
		if (!array_key_exists( 'checked_out', get_class_vars( strtolower(get_class( $this )) ) )) {
			$this->_error = "ВНИМАНИЕ: ".strtolower(get_class( $this ))." не поддерживает снятие блокировки.";
			return false;
		}
	...
	*/
	...
	function publish( $cid=null, $publish=1, $user_id=0 ) {
		...
		if (count( $cid ) < 1) {
			$this->_error = "Нет выделенного объекта";
			return false;
		}
