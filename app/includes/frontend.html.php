...
class modules_html {
	...
	*/
	...
	function modoutput_feed( &$module, &$params, $moduleclass_sfx ) {
		...
		if ( !is_writable( $cacheDir ) ) {	
			$module->content = 'Папка кэша недоступна на запись!';
			return;
		}
	...
	*/
	function modoutput_table( $module, $params, $Itemid, $moduleclass_sfx, $type=0 ) {
		...
		if ( $module->showtitle != 0 ) {
			...
			<tr>
				<th valign="top">
					<?php echo htmlspecialchars(normal( $module->title )); ?>
				</th>
			</tr>
			<?php
		}
	...
	*/
			...
			<div>
				<div>
					<div>
						<?php
						if ($module->showtitle != 0) {
							echo "<h3>" . htmlspecialchars(normal($module->title)) . "</h3>";
						}
	...
	function CustomContent( &$module, $params) {
		...
		if ( $firebots ) {
			$row		= $module;
			$row->text	= normal($module->content);		
		...
		if ( $firebots ) {
			...
			$module->content = normal($row->text);
		}
	}
}
?>