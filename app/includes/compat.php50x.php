...
if (!function_exists('file_put_contents')) {
	function file_put_contents($filename, $content, $flags = null, $resource_context = null)
	{
		...
		if (!is_scalar($content)) {
			trigger_error('file_put_contents() 2-й параметр должен быть строковым или Array', E_USER_WARNING);
			return false;
		}
		...
		if (($fh = @fopen($filename, $mode, $use_inc_path)) === false) {
			trigger_error('file_put_contents() не могу открыть поток: Доступ запрещен', E_USER_WARNING);
			return false;
		}
		...
		if (($bytes = @fwrite($fh, $content)) === false) {
			$errormsg = sprintf('file_put_contents() Не могу записатьe %d байт в %s',
							$length,
							$filename);
		...
		if ($bytes != $length) {
			$errormsg = sprintf('file_put_contents() Только %d из %d байт записано, возможно отсутствие места на диске.',
							$bytes,
							$length);
			trigger_error($errormsg, E_USER_WARNING);
			return false;
