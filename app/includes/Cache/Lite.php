...
class Cache_Lite
{
	...
	*/
	function remove($id, $group = 'default')
	{
		...
		if (!@unlink($this->_file)) {
			$this->raiseError('Cache_Lite : Не могу удалить кэш !', -3);
			return false;
		}
		return true;
	}
	...
	*/
	function clean($group = false)
	{
		...
		if (!($dh = opendir($this->_cacheDir))) {
			$this->raiseError('Cache_Lite : Не могу открыть капку кэша !', -4);
			return false;
		}
	...
	*/
	function _read()
	{
		...
		$this->raiseError('Cache_Lite : Не могу прочесть кэш !', -2);
		return false;
	}
	...
	*/
	function _write($data)
	{
		...
		$this->raiseError('Cache_Lite : Не могу записать в кэш !', -1);
		return false;
	}
	...
	*/
	function _hash($data, $controlType)
	{
		...
		default:
			$this->raiseError('Неизвестный controlType ! (допустимые значения только \'md5\', \'crc32\', \'strlen\')', -5);
		}
	}
}
?>