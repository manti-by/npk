...
class mosToolBar {
	...
	*/
	function custom( $task='', $icon=NULL, $iconOver='', $alt='', $listSelect=true ) {
		if ($listSelect) {
			$href = "javascript:if (document.adminForm.boxchecked.value == 0){ alert('Сделайте выбор из списка для $alt');}else{submitbutton('$task')}";
		} else {
	...
	*/
	function publishList( $task='publish', $alt=_CMN_PUBLISHED ) {
		...
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Сделайте выбор из списка для публикации'); } else {submitbutton('<?php echo $task;?>', '');}" >
				<?php echo $image; ?></a>
		</td>
		<?php
	}
	...
	*/
	function unpublishList( $task='unpublish', $alt=_CMN_UNPUBLISHED ) {
		...
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Сделайте выбор из списка для сокрытия'); } else {submitbutton('<?php echo $task;?>', '');}" >
				<?php echo $image; ?></a>
		</td>
		<?php
	}
	...
	*/
	function archiveList( $task='archive', $alt=_CMN_ARCHIVE ) {
		...
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите материал для архивации'); } else {submitbutton('<?php echo $task;?>', '');}" >
				<?php echo $image; ?></a>
		</td>
		<?php
	}
	...
	*/
	function unarchiveList( $task='unarchive', $alt=_CMN_UNARCHIVE ) {
		...
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите материал для разархивации'); } else {submitbutton('<?php echo $task;?>', '');}" >
				<?php echo $image; ?></a>
		</td>
		<?php
	}
	...
	*/
	function editList( $task='edit', $alt=_E_EDIT ) {
		...
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Сделайте выбор из списка для редактирования'); } else {submitbutton('<?php echo $task;?>', '');}" >
				<?php echo $image; ?></a>
		</td>
		<?php
	}
	...
	*/
	function media_manager( $directory = '' ) {
		$image = mosAdminMenus::ImageCheck( 'upload_f2.png', '/images/', NULL, NULL, 'Загрузить картинку', 'uploadPic', 1 );
		?>
		<td>
