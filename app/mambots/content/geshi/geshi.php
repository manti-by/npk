...
class GeSHi
{
    ...
    var $error_messages = array(
        ...
        GESHI_ERROR_NO_SUCH_LANG => 'GeSHi не может найти языковый файл {LANGUAGE} (использован путь {PATH})',
        GESHI_ERROR_FILE_NOT_READABLE => 'Требуемый файл load_from_file недоступен на чтение',
        GESHI_ERROR_INVALID_HEADER_TYPE => 'Спецификация заголовка недопустима',
        GESHI_ERROR_INVALID_LINE_NUMBER_TYPE => 'Номер строки является недопустимым'
    );
	...
	function error ()
	{
		if ($this->error) {
			...
			return "<br /><strong>GeSHi ошибка:</strong> $msg (code $this->error)<br />";
		}
		return false;
	}
	...
	function get_stylesheet ($economy_mode = true)
	{
		...
		if (!$economy_mode) {
			$stylesheet = "/**\n * GeSHi - динамически сгенерированная таблица стилей\n * --------------------------------------\n * Динамически сгенерировано для {$this->language}\n * Класс CSS: {$this->overall_class}, CSS id: {$this->overall_id}\n * GeSHi (c) Nigel McNie 2004 (http://qbnz.com/highlighter)\n */\n";
 		} else {
