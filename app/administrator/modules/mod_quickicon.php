...
if (!defined( '_JOS_QUICKICON_MODULE' )) {
	...
	<div id="cpanel">
		...
		quickiconButton( $link, 'module.png', 'Добавить материал' );
	
		$link = 'index2.php?option=com_content&sectionid=0';
		quickiconButton( $link, 'addedit.png', 'Материалы' );
	
		$link = 'index2.php?option=com_typedcontent';
		quickiconButton( $link, 'addedit.png', 'Статичные материалы' );
		...
		quickiconButton( $link, 'mediamanager.png', 'Медиа-менеджер' );
		...
		if ( $my->gid > 24 ) {
			...
			quickiconButton( $link, 'langmanager.png', 'Локализации' );
	}
		...
		if ( $my->gid > 24 ) {
			...
			quickiconButton( $link, 'config.png', 'Общие настройки' );
		}
