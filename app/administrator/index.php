...
if (isset( $_POST['submit'] )) {
	...
	if($pass == NULL) {
		echo "<script>alert('Введите пароль!'); document.location.href='index.php?mosmsg=Please enter a password'</script>\n";
		exit();
	}
	...
	if (@$my->id) {
		...
		if ( strcmp( $hash, $cryptpass ) || !$acl->acl_check( 'administration', 'login', 'users', $my->usertype ) ) {
			mosErrorAlert("Неверные имя пользователя, пароль, или Уровень доступа.  Попробуйте еще раз.", "document.location.href='index.php'");
		}
		...
		mosErrorAlert("Неверные имя пользователя или пароль. Попробуйте еще раз", "document.location.href='index.php?mosmsg=Неверные имя пользователя или пароль. Попробуйте еще раз'");
	}
} else {
	initGzip();
