...
function mosLoadCustomModule( &$module, &$params ) {
	...
	if ( $rssurl ) {
		if (!is_writable( $cachePath )) {
			echo '<tr>';
			echo '<td>Сделайте папку кэша доступнойна запись!</td>';
			echo '</tr>';
		} else {
			...
			if ( $success )	{
				...
				for ($i = 0; $i < $totalChannels; $i++) {
					...
					for ($j = 0; $j < $totalItems; $j++) {
						...
						echo normal($item_title) .'</a></strong> - '. normal($text) .'</td>';
						echo '</tr>';
					}
				}
			}
...
function josSecurityCheck($width='95%') {
	...
	if ( ini_get('magic_quotes_gpc') != '1' ) {
		$wrongSettingsTexts[] = 'PHP magic_quotes_gpc установлена в `OFF` вместо `ON`';
	}
	if ( ini_get('register_globals') == '1' ) {
		$wrongSettingsTexts[] = 'PHP register_globals установлена в `ON` вместо `OFF`';
	}
	if ( RG_EMULATION != 0 ) {
		$wrongSettingsTexts[] = 'Joomla! &quot;Register Globals Emulation&quot; установлена в `ON`. &nbsp; Для запрещения Register Globals Emulation, зайдите в меню Сайт -> Общие настройки -> Сервер, выберите `Откл`, и сохраните.<br /><span style="font-weight: normal; font-style: italic; color: #666;">Register Globals Emulation установлено в `Вкл.` по умолчанию для большей совместимости.</span>';
	}
	...
	if ( count($wrongSettingsTexts) ) {
		...
		<div style="clear: both; margin: 3px; margin-top: 10px; padding: 5px 15px; display: block; float: left; border: 1px solid #cc0000; background: #ffffcc; text-align: left; width: <?php echo $width;?>;">
			...
			<p style="color: #666;">
				Для информации зайдите на <a href="http://www.joomla.org/security10" target="_blank" style="color: blue; text-decoration: underline">сервер официальных сообщений Joomla! Server Security </a>.
			</p>
		</div>
		<?php
	}
