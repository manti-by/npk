...
class mosMenuBar {
	...
	*/
	function custom( $task='', $icon='', $iconOver='', $alt='', $listSelect=true ) {
		...
		if ($listSelect) {
			$href = "javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект в списке для $alt');}else{submitbutton('$task')}";
		} else {
	...
	*/
	function customX( $task='', $icon='', $iconOver='', $alt='', $listSelect=true ) {
		...
		if ($listSelect) {
			$href = "javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект в списке для $alt');}else{hideMainMenu();submitbutton('$task')}";
		} else {
	...
	* @param string An override for the alt text
	*/
	function publish( $task='publish', $alt='Публикация' ) {
	...
	* @param string An override for the alt text
	*/
	function publishList( $task='publish', $alt='Публикация' ) {
		$image2 = mosAdminMenus::ImageCheckAdmin( 'publish_f2.png', '/administrator/images/', NULL, NULL, $alt, $task, 1, 'middle', $alt );
		?>
	 	<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект для публикации'); } else {submitbutton('<?php echo $task;?>', '');}">
				<?php echo $image2; ?>
				<br /><?php echo $alt; ?></a>
		</td>
	 	<?php
	...
	* @param string An override for the alt text
	*/
	function makeDefault( $task='default', $alt='Умолчание' ) {
		$image2 = mosAdminMenus::ImageCheckAdmin( 'publish_f2.png', '/administrator/images/', NULL, NULL, $alt, $task, 1, 'middle', $alt );
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект для установки по умолчанию'); } else {submitbutton('<?php echo $task;?>', '');}">
				<?php echo $image2; ?>
				<br /><?php echo $alt; ?></a>
		</td>
		<?php
	...
	*/
	function assign( $task='assign', $alt='Назначить' ) {
		...
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект для назначения'); } else {submitbutton('<?php echo $task;?>', '');}">
				<?php echo $image2; ?>
				<br /><?php echo $alt; ?></a>
		</td>
		<?php
	...
	*/
	function unpublishList( $task='unpublish', $alt='Скрыть' ) {
		...
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект для снятия с публикации'); } else {submitbutton('<?php echo $task;?>', '');}">
				<?php echo $image2; ?>
				<br /><?php echo $alt; ?></a>
		</td>
		<?php
	...
	*/
	function archiveList( $task='archive', $alt='В&nbsp;архив' ) {
		...
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект для архивирования'); } else {submitbutton('<?php echo $task;?>', '');}">
				<?php echo $image2; ?>
				<br /><?php echo $alt; ?></a>
		</td>
		<?php
	...
	*/
	function unarchiveList( $task='unarchive', $alt='Из&nbsp;архива' ) {
		...
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект для разархивирования'); } else {submitbutton('<?php echo $task;?>', '');}">
				<?php echo $image2; ?>
				<br /><?php echo $alt; ?></a>
		</td>
		<?php
	...
	* @param string An override for the alt text
	*/
	function editList( $task='edit', $alt='Правка' ) {
		$image2 = mosAdminMenus::ImageCheckAdmin( 'edit_f2.png', '/administrator/images/', NULL, NULL, $alt, $task, 1, 'middle', $alt );
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект для редактирования'); } else {submitbutton('<?php echo $task;?>', '');}">
				<?php echo $image2; ?>
				<br /><?php echo $alt; ?></a>
		</td>
		<?php
	...
	* @param string An override for the alt text
	*/
	function editListX( $task='edit', $alt='Правка' ) {
		$image2 = mosAdminMenus::ImageCheckAdmin( 'edit_f2.png', '/administrator/images/', NULL, NULL, $alt, $task, 1, 'middle', $alt );
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект из списка для редактирования'); } else {hideMainMenu();submitbutton('<?php echo $task;?>', '');}">
				<?php echo $image2; ?>
				<br /><?php echo $alt; ?></a>
		</td>
		<?php
	...
	* @param string An override for the alt text
	*/
	function editHtml( $task='edit_source', $alt='Правка&nbsp;HTML' ) {
		$image2 = mosAdminMenus::ImageCheckAdmin( 'html_f2.png', '/administrator/images/', NULL, NULL, $alt, $task, 1, 'middle', $alt );
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект из списка для редактирования'); } else {submitbutton('<?php echo $task;?>', '');}">
				<?php echo $image2; ?>
				<br /><?php echo $alt; ?></a>
		</td>
		<?php
	...
	* @param string An override for the alt text
	*/
	function editHtmlX( $task='edit_source', $alt='Правка&nbsp;HTML' ) {
		$image2 = mosAdminMenus::ImageCheckAdmin( 'html_f2.png', '/administrator/images/', NULL, NULL, $alt, $task, 1, 'middle', $alt );
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект из списка для редактирования'); } else {hideMainMenu();submitbutton('<?php echo $task;?>', '');}"">
	...
	* @param string An override for the alt text
	*/
	function editCss( $task='edit_css', $alt='Правка&nbsp;CSS' ) {
	...
	* @param string An override for the alt text
	*/
	function editCssX( $task='edit_css', $alt='Правка&nbsp;CSS' ) {
		$image2 = mosAdminMenus::ImageCheckAdmin( 'css_f2.png', '/administrator/images/', NULL, NULL, $alt, $task, 1, 'middle', $alt );
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект из списка для редактирования'); } else {hideMainMenu();submitbutton('<?php echo $task;?>', '');}">
	...
	*/
	function deleteList( $msg='', $task='remove', $alt='Удалить' ) {
		...
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект из списка для удаления'); } else if (confirm('Are you sure you want to delete selected items? <?php echo $msg;?>')){ submitbutton('<?php echo $task;?>');}">
				<?php echo $image2; ?>
				<br /><?php echo $alt; ?></a>
		</td>
		<?php
	...
	*/
	function deleteListX( $msg='', $task='remove', $alt='Удалить' ) {
		...
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект из списка для удаления'); } else if (confirm('Are you sure you want to delete selected items? <?php echo $msg;?>')){ hideMainMenu();submitbutton('<?php echo $task;?>');}">
				<?php echo $image2; ?>
				<br /><?php echo $alt; ?></a>
		</td>
		<?php
	...
	*/
	function trash( $task='remove', $alt='В&nbsp;корзину', $check=true ) {
		...
		if ( $check ) {
			$js = "javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект из списка для помещения в корзину'); } else { submitbutton('$task');}";
		} else {
	...
	*/
	function preview( $popup='', $updateEditors=false ) {
		global $database;
		$image2 = mosAdminMenus::ImageCheckAdmin( 'preview_f2.png', '/administrator/images/', NULL, NULL, 'Просмотр', 'preview', 1, 'middle', 'Просмотр' );
	...
	*/
	function help( $ref, $com=false ) {
		...
		$image2 	= mosAdminMenus::ImageCheckAdmin( 'help_f2.png', '/administrator/images/', NULL, NULL, 'Справка', 'help', 1, 'middle', 'Справка' );
	...
	*/
	function media_manager( $directory='', $alt='Загрузка' ) {
		global $database, $mainframe;
		
		$cur_template = $mainframe->getTemplate();
		
		$image2 = mosAdminMenus::ImageCheckAdmin( 'upload_f2.png', '/administrator/images/', NULL, NULL, 'Загрузка картинки', 'uploadPic', 1 );
		?>
		<td>
