...
class HTML_messages {
	...
	function editConfig( &$vars, $option) {	
		...
		<tr>
			...
			<td> 
				<input title="Максимальная длина 5 знаков" type="text" name="vars[auto_purge]" size="5" value="<?php echo $vars['auto_purge']; ?>" class="inputbox" /> дней спустя
			</td>
		</tr>
		</table>				
	...
	function newMessage($option, $recipientslist, $subject ) {
		...
		function submitbutton(pressbutton) {
			...
			alert( "Вы должны заполнить тему сообщения." );
		} else if (form.message.value == "") {
			alert( "Вы должны заполнить само сообщение." );
		} else if (getSelectedValue('adminForm','user_id_to') < 1) {
			alert( "Вы должны выбрать адресата." );
			} else {
				submitform( pressbutton );
			}
		}
		...
		<tr>
			...
			<td>
				<input title="Максимальная длина = 100 знаков" type="text" name="subject" size="50" maxlength="100" class="inputbox" value="<?php echo htmlspecialchars( $subject, ENT_QUOTES ); ?>"/>
			</td>
		</tr>
		<tr>
			<td valign="top">
