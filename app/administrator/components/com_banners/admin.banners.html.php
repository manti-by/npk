...
class HTML_banners {
		...
		for ($i=0, $n=count( $rows ); $i < $n; $i++) {
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				if ( $row->checked_out && ( $row->checked_out != $my->id ) ) {
					...
					?>
					<a href="<?php echo $link; ?>" title="Редактировать баннер">
					<?php echo $row->name; ?>
					</a>
					<?php
				}
	...
	function bannerForm( &$_row, &$lists, $_option ) {
		...
		function submitbutton(pressbutton) {
			...
			// do field validation
			if (form.name.value == "") {
				alert( "Введите имя баннера." );
			} else if (getSelectedValue('adminForm','cid') < 1) {
				alert( "Выберите клиента баннера." );
			} else if (!getSelectedValue('adminForm','imageurl')) {
				alert( "Выберите картинку баннера." );
		...
		<tr>
			...
			Баннер:
			<small>
			<?php echo $_row->cid ? 'Изменить' : 'Новый';?>
			</small>
			</th>
		</tr>
		</table>
		...
		<tr>
			...
			&nbsp;&nbsp;&nbsp;&nbsp;
			Без ограничения <input type="checkbox" name="unlimited" <?php echo $unlimited;?> />
			</td>
		</tr>
		<tr>
			<td>
		...
		<tr >
			...
			&nbsp;&nbsp;&nbsp;&nbsp;
			<input name="reset_hits" type="button" class="button" value="Сбросить клики" onClick="submitbutton('resethits');" />
			</td>
		</tr>
		<tr>
			<td valign="top">
...
class HTML_bannerClient {
		...
		for ($i=0, $n=count( $rows ); $i < $n; $i++) {
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				if ( $row->checked_out && ( $row->checked_out != $my->id ) ) {
					...
					?>
					<a href="<?php echo $link; ?>" title="Редактировать клиента">
					<?php echo $row->name; ?>
					</a>
					<?php
				}
	...
	function bannerClientForm( &$row, $option ) {
		...
		<tr>
			...
			Клиент баннера:
			<small>
			<?php echo $row->cid ? 'Изменить' : 'Новый';?>
			</small>
			</th>
		</tr>
		</table>
