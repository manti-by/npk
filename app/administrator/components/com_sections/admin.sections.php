...
function editSection( $uid=0, $scope='', $option ) {
	...
	if ($row->isCheckedOut( $my->id )) {
		$msg = 'Раздел '. $row->title .' сейчас редактируется другим администратором';
	...
	$types[] = mosHTML::makeOption( 'content_section', 'Раздел - список' );
	$types[] = mosHTML::makeOption( 'content_blog_section', 'Раздел - блог' );
	$types[] = mosHTML::makeOption( 'content_archive_section', 'Раздел - блог архива' );
...
function saveSection( $option, $scope, $task ) {
	...
	switch ( $task ) {
		...
		case 'apply':
			$msg = 'Изменения в разделе сохранены';
...
function removeSections( $cid, $scope, $option ) {
	...
	if (count( $err )) {
		$cids = implode( ', ', $err );
		$msg = 'Раздел(ы): '. $cids .' не может быть удален, поскольку содержит категории';
		mosRedirect( 'index2.php?option='. $option .'&scope='. $scope, $msg );
	}
	...
	$names = implode( ', ', $name );
	$msg = 'Раздел(ы): '. $names .' успешно удален';
	mosRedirect( 'index2.php?option='. $option .'&scope='. $scope, $msg );
}
...
function publishSections( $scope, $cid=null, $publish=1, $option ) {
	...
	if ( $publish ) {
		if ( !$count ){
			echo "<script> alert('Не могу опубликовать пустой раздел $count'); window.history.go(-1);</script>\n";
			return;
		}
	}
...
function menuLink( $id ) {
	...
	$msg = $name .' ( '. $menutype .' ) в меню: '. $menu .' успешно создан';
