...
function editContent( $uid=0, $sectionid=0, $option ) {
	...
	if ($uid) {
		...
		if ($row->state < 0) {
			mosRedirect( 'index2.php?option=com_content&sectionid='. $row->sectionid, 'Вы не можете редактировать архивированный материал' );
		}
	}

	// fail if checked out not by 'me'
	if ($row->checked_out && $row->checked_out != $my->id) {
		mosRedirect( 'index2.php?option=com_content', 'Модуль '. $row->title .' сейчас редактируется другим администратором' );
	}
	...
	$database->setQuery( $query );
	if ( $sectionid == 0 ) {
		$sections[] = mosHTML::makeOption( '-1', '- Выбор раздела -', 'id', 'title' );
	...
	$sectioncategories[-1][] 	= mosHTML::makeOption( '-1', '- Выбор категории -', 'id', 'name' );
	...
	foreach($sections as $section) {
 	...
 	// get list of categories
  	if ( !$row->catid && !$row->sectionid ) {
 		$categories[] 		= mosHTML::makeOption( '-1', '- Выбор категории -', 'id', 'name' );
...
function changeContent( $cid=null, $state=0, $option ) {
	...
	switch ( $state ) {
		case -1:
			$msg = $total .' объектов успешно архивировано';
			break;

		case 1:
			$msg = $total .' объектов успешно опубликовано';
			break;
			
		case 0:				
		default:
			if ( $task == 'unarchive' ) {
				$msg = $total .' объектов успешно разархивировано';
			} else {
				$msg = $total .' объектов успешно скрыто';
			}
			break;
	}
...
function removeContent( &$cid, $sectionid, $option ) {
	...
	$msg = "Послано в корзину материалов: ".$total;
...
function moveSectionSave( &$cid, $sectionid, $option ) {
	...
	$msg = $total. ' материалов успешно перемещены в раздел: '. $section .', Категория: '. $category;
...
function copyItemSave( $cid, $sectionid, $option ) {
	...
	if ( !$newsect && !$newcat ) {
		mosRedirect( 'index.php?option=com_content&sectionid='. $sectionid .'&mosmsg=Произошла ошибка' );
	}
	...
	$msg = $total. ' обюъект(ов) успешно скопировано в раздел: '. $section .', категория: '. $category;
	mosRedirect( 'index2.php?option='. $option .'&sectionid='. $sectionid .'&mosmsg='. $msg );
}
...
function menuLink( $redirect, $id ) {
	...
	$msg = $link .' (Ссылка - материал) в меню: '. $menu .' успешно создана';
	mosRedirect( 'index2.php?option=com_content&sectionid='. $redirect .'&task=edit&hidemainmenu=1&id='. $id, $msg );
}
