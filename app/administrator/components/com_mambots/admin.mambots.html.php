...
class HTML_modules {
	...
	*/
	function showMambots( &$rows, $client, &$pageNav, $option, &$lists, $search ) {
		...
		<tr>
			<th class="modules">
			Менеджер мамботов <small><small>[ <?php echo $client == 'admin' ? 'Админка' : 'Сайт';?> ]</small></small>
			</th>
			<td>
			Фильтр:
			</td>
          ...
          ?>
			...
			<td width="40%">
				...
				<tr>
					...
					if ( $row->id ) {
						include_once ($GLOBALS['mosConfig_absolute_path'] .'/administrator/includes/andyr_lib.php');
						echo normal ($params->render());
					} else {
						echo '<i>Нет патаметров</i>';
					}
					?>
					</td>
				</tr>
