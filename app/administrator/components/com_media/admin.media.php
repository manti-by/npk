...
if (is_int(strpos ($listdir, "..")) && $listdir != '') {
	mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], ":) Взломом занимаешься, да? :Р" );
}
...
switch ($task) {
	...
	case 'newdir':
		if (ini_get('safe_mode')=='On') {
			mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], "Создание каталогов не разрешено пока мы в SAFE MODE , поскольку могут быть проблемы :)" );
		} else {
...
function create_folder($dirPath) {
	...
	if(strlen($folder_name) >0) {
		if (eregi("[^0-9a-zA-Z_]", $folder_name)) {
			mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], "Имя каталога может содержать только буквенно-цифровые символы и без пробелов." );
		}
...
function delete_folder($listdir) {
	...
	if ($entry_count < 1) {
		...
		echo '<font color="red">Не буду удалять: там что-то есть внутри! :)</font>';
	}
}
...
function do_upload($file, $dest_dir) {
	...
	if (empty($file['name'])) {
		mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], "Не выбран файл" );
	}
	if (file_exists($dest_dir.$file['name'])) {
		mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], "Загрузка неудачна. Файл уже существует" );
	}
    ...
    if(!$noMatch){
		mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], 'Тип этого файла не поддерживается' );
    }

	if (!move_uploaded_file($file['tmp_name'], $dest_dir.strtolower($file['name']))){
		mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], "Загрузка неудачна" );
	} else {
		mosChmod($dest_dir.strtolower($file['name']));
		mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], "Загрузка завершена" );
	}
