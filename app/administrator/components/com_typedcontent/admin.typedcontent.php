...
function view( $option ) {
	...
	$ordering[] = mosHTML::makeOption( 'c.ordering ASC', 'Порядок - возрастание' );
	$ordering[] = mosHTML::makeOption( 'c.ordering DESC', 'Порядок - убывание' );
	$ordering[] = mosHTML::makeOption( 'c.id ASC', 'ID - возрастание' );
	$ordering[] = mosHTML::makeOption( 'c.id DESC', 'ID - убывание' );
	$ordering[] = mosHTML::makeOption( 'c.title ASC', 'Заголовок - возрастание' );
	$ordering[] = mosHTML::makeOption( 'c.title DESC', 'Заголовок - убывание' );
	$ordering[] = mosHTML::makeOption( 'c.created ASC', 'Дата - возрастание' );
	$ordering[] = mosHTML::makeOption( 'c.created DESC', 'Дата - убывание' );
	$ordering[] = mosHTML::makeOption( 'z.name ASC', 'Автор - возрастание' );
	$ordering[] = mosHTML::makeOption( 'z.name DESC', 'Автор - убывание' );
	$ordering[] = mosHTML::makeOption( 'c.state ASC', 'Публикация - возрастание' );
	$ordering[] = mosHTML::makeOption( 'c.state DESC', 'Публикация - убывание' );
	$ordering[] = mosHTML::makeOption( 'c.access ASC', 'Доступ - возрастание' );
	$ordering[] = mosHTML::makeOption( 'c.access DESC', 'Доступ - убывание' );
...
function edit( $uid, $option ) {
	...
	if ($uid) {
		...
		if ($row->isCheckedOut( $my->id )) {
			mosErrorAlert( "Модуль ".$row->title." сейчас редактируется другим администратором" );
		}
...
function save( $option, $task ) {
	...
	switch ( $task ) {
		...
		case 'save':
			$msg = 'Статичный материал сохранен';
			mosRedirect( 'index2.php?option='. $option, $msg );
			break;

		case 'apply':
		default:
			$msg = 'Изменения статичного материала сохранены';
...
function trash( &$cid, $option ) {
	...
	$msg = $total ." материалов удалено в корзину";
	mosRedirect( 'index2.php?option='. $option, $msg );
}
...
function changeState( $cid=null, $state=0, $option ) {
	...
	if ( $state == "1" ) {
		$msg = $total ." материал успешно опубликован";
	} else if ( $state == "0" ) {
		$msg = $total ." материал успешно скрыт";
	}
...
function resethits( $option, $id ) {
	...
	$msg = 'Счетчик хитов успешно обнулен';
	mosRedirect( 'index2.php?option='. $option .'&task=edit&hidemainmenu=1&id='. $row->id, $msg );
}
...
function menuLink( $option, $id ) {
	...
	$msg = $link .' (Ссылка - статичный материал) в меню: '. $menu .' успешно создан';
