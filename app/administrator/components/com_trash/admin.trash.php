...
function viewTrash( $option ) {
	...
	if ($catid=="content") {
		...
		for ( $i = 0; $i < $num-1; $i++ ) {
			if ( ( $content[$i]->sectionid == 0 ) && ( $content[$i]->catid == 0 ) ) {
				$content[$i]->sectname = 'Статичные материалы';
			}
		}
	} else {
	...
	$listselect = array();
	$listselect[] = mosHTML::makeOption( 'content', 'Объекты - материалы' );
	$listselect[] = mosHTML::makeOption( 'menu', 'Объекты - меню' );
	$selected = "all";
...
function deleteTrash( $cid, $option ) {
	...
	$msg = "Успешно удалено объектов: ".$total ;
...
function restoreTrash( $cid, $option ) {
	...
	$msg = "Успешно восстановлено объектов: ".$total ;
	mosRedirect( "index2.php?option=$option&mosmsg=". $msg ."" );
}
?>