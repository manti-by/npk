...
class JCommentsMigrationTool
{
	...
	function doImport( $option )
	{
		...
		JCommentsRedirect( 'index2.php?option=' . $option . '&task=import' , $message );
	}
	...
	function importmXcomment()
	{
		...
		foreach( $rows as $row) {
			...
			$comment->parent	= isset($row->parentid) ? ($maxid + $row->parentid) : 0;
	...
	function importAkoBook()
	{
		...
		foreach( $rows as $row) {
			...
			$comment->username 	= JCommentsMigrationTool::processName( ($row->username != '' ? $row->username : $row->gbname) );
