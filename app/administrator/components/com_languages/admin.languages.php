...
function publishLanguage( $p_lname, $option ) {
	...
	if ($fp = fopen('../configuration.php','w')){
		...
		fclose($fp);
		mosRedirect('index2.php?option=com_languages',"Язык успешно сменен! $p_lname");
	} else {
...
function removeLanguage( $cid, $option, $client = 'admin' ) {
	...
	if ($cur_language == $cid) {
		mosErrorAlert( "Нельзя удалить активный язык." );
	}
...
function editLanguageSource( $p_lname, $option) {
	...
	if ($fp = fopen( $file, "r" )) {
		...
		mosRedirect( "index2.php?option=$option&mosmsg=Операция неудачна: Не могу открыть файл $file" );
	}
}
...
function saveLanguageSource( $option ) {
	...
	if (!$language) {
		mosRedirect( "index2.php?option=$option&mosmsg=Операция неудачна: Не выбран язык." );
	}
	if (!$filecontent) {
		mosRedirect( "index2.php?option=$option&mosmsg=Операция неудачна: Содержимое отсутствует." );
	}
	...
	if (is_writable( $file ) == false) {
		mosRedirect( "index2.php?option=$option&mosmsg=Операция неудачна: Файл защищен от записи." );
	}
	...
	if ($fp = fopen ($file, "w")) {
		...
		mosRedirect( "index2.php?option=$option&mosmsg=Операция неудачна: Не могу открыть файл на запись." );
	}
}
?>