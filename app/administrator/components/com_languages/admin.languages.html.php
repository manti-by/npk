...
class HTML_languages {
	...
	function showLanguages( $cur_lang, &$rows, &$pageNav, $option ) {
		...
		<tr>
			...
			Менеджер локализаций <small><small>[ Сайт ]</small></small>
			</th>
		</tr>
		</table>
		...
		for ($i=0, $n=count( $rows ); $i < $n; $i++) {
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				<td align=center>
				<?php echo normal ($row->author); ?>
				</td>
				<td align=center>
				<?php echo normal ($row->authorEmail); ?>
				</td>
			</tr>
		<?php
		}
	...
	function editLanguageSource( $language, &$content, $option ) {
		...
		<tr>
			<td width="270"><table class="adminheading"><tr><th class="langmanager">Редактор языков</th></tr></table></td>
			<td width="240">
				<span class="componentheading"><?php echo $language; ?>.php :
				<b><?php echo is_writable($language_path) ? '<font color="green"> доступен на запись</font>' : '<font color="red"> недоступен на запись</font>' ?></b>
				</span>
			</td>
<?php
...
?>
			<td>
				...
				<label for="enable_write">Снять зашиту от записи на момент внесения изменений</label>
			</td>
<?php
				} // if
			} // if
