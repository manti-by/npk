...
function messageForm( $option ) {
	...
	$gtree = array(
	mosHTML::makeOption( 0, '- Всем группам пользователей -' )
	);
...
function sendMail() {
	...
	if (!$message_body || !$subject || $gou === null) {
		mosRedirect( 'index2.php?option=com_massmail&mosmsg=Заполните форму правильно' );
	}
	...
	$msg = 'E-mail отправлен '. count( $rows ) .' пользователям';
