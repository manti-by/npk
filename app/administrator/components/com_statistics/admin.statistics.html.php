...
class HTML_statistics {
	function show( &$browsers, &$platforms, $tldomains, $bstats, $pstats, $dstats, $sorts, $option ) {
		...
		<tr>
			<th class="browser">Браузеры, OS, Домены</th>
		</tr>
		</table>
	...
	function pageImpressions( &$rows, $pageNav, $option, $task ) {
		...
		<tr>
			<th width="100%" class="impressions">Статистика обращения к материалам</th>
		</tr>
		</table>
		...
		<tr>
			...
			<th align="center" nowrap="nowrap">Количество обращений</th>
		</tr>
		<?php
	...
	function showSearches( &$rows, $pageNav, $option, $task, $showResults ) {
		...
		<tr>
			<th class="searchtext">
				...
				<span class="componentheading">состояние :
					<?php echo $mainframe->getCfg( 'enable_log_searches' ) ? '<b><font color="green">Enabled</font></b>' : '<b><font color="red">Запрещена</font></b>' ?>
				</span>
			</th>
			<td align="right">
				<?php
				if ( !$showResults ) {
					echo mosWarning('Активация этого режима может значительно замедлить работу и даже заблокировать сайт, поскольку система начинает обрабатывать огомное количество запросов.');
				}
				?>
			</td>
			<td align="right">
