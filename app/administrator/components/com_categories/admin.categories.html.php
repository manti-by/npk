	...
	function show( &$rows, $section, $section_name, &$pageNav, &$lists, $type ) {
		...
		<tr>
			...
			if ( $section == 'content') {
				?>
				<th class="categories">
				Менеджер категорий <small><small>[ материалы: Все ]</small></small>
				</th>
				<td width="right">
				...
				?>
				<th class="categories">
				Менеджер категорий <small><small>[ <?php echo $section_name;?> ]</small></small>
				</th>
				<?php
			}
			?>
		...
		<tr>
			<td valign="top" width="60%">
				...
				<tr>
					...
					<td colspan="2">
					<input class="text_area" type="text" name="name" value="<?php echo stripslashes( $row->name ); ?>" size="50" maxlength="255" title="Длинное имя для заголовков" />
					</td>
				</tr>
				<tr>
					<td>
			...
			<td valign="top" width="40%">
				...
				if ( $row->id > 0 ) {
					...
					<tr>
						<td colspan="2">
						Будет создан новый пункт в указанном меню
						<br /><br />
						</td>
					<tr>
					<tr>
					...
					<tr>
						<td>
						Ссылка на меню станет доступна после записи
						</td>
					</tr>
					</table>
					<?php
	...
	function moveCategorySelect( $option, $cid, $SectionList, $items, $sectionOld, $contents, $redirect ) {
		...
		function submitbutton(pressbutton) {
			...
			if (!getSelectedValue( 'adminForm', 'sectionmove' )) {
				alert( "Выберите Раздел в который надо переместить Категорию" );
			} else {
				submitform( pressbutton );
			}
		}
		...
		<tr>
			...
			<strong>Перенести в раздел:</strong>
			<br />
			...
			<strong>Перемещаемые материалы:</strong>
			<br />
			<?php
			echo "<ol>";
			...
			<td valign="top">
			Перемещение показанных категорий
			<br />
			и всех материалов внутри категории
			<br />
			в выбранный раздел.
			</td>.
		</tr>
	...
	*/
	function copyCategorySelect( $option, $cid, $SectionList, $items, $sectionOld, $contents, $redirect ) {
		...
		<tr>
			...
			<td valign="top" width="20%">
			<strong>Копируемые материалы:</strong>
			<br />
			<?php
			echo "<ol>";
			foreach ( $contents as $content ) {
			...
			<td valign="top">
			Копирование указанных категорий
			<br />
			и всех материалов внутри категории
			<br />
			в выбранный раздел.
