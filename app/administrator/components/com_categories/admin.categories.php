...
function showCategories( $section, $option ) {
	...
	if (intval( $section ) > 0) {
		...
		$section_name = 'Материалы: '. $section_name;
...
function editCategory( $uid=0, $section='' ) {
	...
	if (!$sections && $type != 'other') {
		echo "<script> alert('Требуется как минимум один Раздел для того, чтобы создать Категорию'); window.history.go(-1); </script>\n";
		exit();
	}
	...
	if ($row->checked_out && $row->checked_out != $my->id) {
		mosRedirect( 'index2.php?option=categories&section='. $row->section, 'Категория '. $row->title .' сейчас редактируется другим администратором' );
	}
	...
	if ( $uid ) {
		...
		switch ( $row->section ) {
			case 'com_weblinks':
				...
				$link 	= 'Таблица - категория ссылок';
				break;
			
			case 'com_newsfeeds':
				$and 	= "\n AND type = 'newsfeed_category_table'";
				$link 	= 'Таблица - категория RSS-лент';
				break;
			
			case 'com_contact_details':
				$and 	= "\n AND type = 'contact_category_table'";
				$link 	= 'Таблица - категория контактов';
				break;
			...
			for( $i = 0; $i < $count; $i++ ) {
				switch ( $menus[$i]->type ) {
					case 'content_category':
						$menus[$i]->type = 'Таблица - категория материалов';
						break;

					case 'content_blog_category':
						$menus[$i]->type = 'Блог - категория материалов';
						break;

					case 'content_archive_category':
						$menus[$i]->type = 'Блог - категория архивных материалов';
						break;
				}
			}
			$lists['links']	= 1;
	...
	$types[] = mosHTML::makeOption( '', '- Выбор типа -' );
	if ($row->section == 'com_contact_details') {
		$types[] = mosHTML::makeOption( 'contact_category_table', 'Таблица категории контактов' );
	} else
	if ($row->section == 'com_newsfeeds') {
		$types[] = mosHTML::makeOption( 'newsfeed_category_table', 'Таблица категории RSS-лент' );
	} else
	if ($row->section == 'com_weblinks') {
		$types[] = mosHTML::makeOption( 'weblink_category_table', 'Таблица категории ссылок' );
	} else {
		$types[] = mosHTML::makeOption( 'content_category', 'Таблица категории материалов' );
		$types[] = mosHTML::makeOption( 'content_blog_category', 'Блог категории материалов' );
		$types[] = mosHTML::makeOption( 'content_archive_category', 'Блог категории архивных материалов' );
	} // if
	...
	if ( $row->section > 0 || $row->section == 'content' ) {
		...
		$folders[] 	= mosHTML::makeOption( '*2*', ' Использовать установки раздела '  );
		$folders[] 	= mosHTML::makeOption( '*#*', '---------------------' );
		$folders[] 	= mosHTML::makeOption( '*1*', 'Все'  );
		$folders[] 	= mosHTML::makeOption( '*0*', 'Нет' );
		$folders[] 	= mosHTML::makeOption( '*#*', '---------------------' );
		$folders[] 	= mosHTML::makeOption( '/' );
...
function saveCategory( $task ) {
	...
	switch ( $task ) {
		...
		case 'apply':
			$msg = 'Изменения категории сохранены';
...
function removeCategories( $section, $cid ) {
	...
	if (count( $err )) {
		$cids = implode( "\', \'", $err );
		$msg = 'Категория(и): '. $cids .' не может быть удалена, поскольку содержит записи (проверьте корзину)';
...
function moveCategorySave( $cid, $sectionOld ) {
	...
	if ( !$sectionMove ) {
		mosRedirect( 'index.php?option=com_categories&mosmsg=Произошла ошибка' );
	}
	...
	$msg = ( (count($cid) - 1) ? 'Категорий' : 'Категория' ) .' перемещены в '. $sectionNew->name;
...
function copyCategorySave( $cid, $sectionOld ) {
	...
	foreach( $cid as $id ) {
		...
		$category->id 		= NULL;
		$category->title 	= 'Копия '. $category->title;
		$category->name 	= 'Копия '. $category->name;
	...
	$msg = ( (count($cid) - 1) ? 'Категорий' : 'Категория' ) .' скопировано '. $sectionNew->name;
	mosRedirect( 'index2.php?option=com_categories&section='. $sectionOld .'&mosmsg='. $msg );
}
...
function menuLink( $id ) {
	...
	switch ( $type ) {
		case 'content_category':
			...
			$menutype	= 'Таблица категории материалов';
			break;

		case 'content_blog_category':
			$link 		= 'index.php?option=com_content&task=blogcategory&id='. $id;
			$menutype	= 'Блог категории материалов';
			break;
