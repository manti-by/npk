	...
	function show ( $option, $menus, $pageNav ) {
		...
		for ($m = $start; $m < $start+$count; $m++) {
			...
			<tr class="<?php echo "row". $k; ?>">
				...
				<td>
				<a href="<?php echo $link; ?>" title="Редактировать имя меню">
				<?php echo $menu->type; ?>
				</a>
				</td>
				<td align="center">
				<a href="<?php echo $linkA; ?>" title="Редактировать пункты меню">
	...
	*/
	function edit ( &$row, $option ) {
		...
		function submitbutton(pressbutton) {
			...
			if (pressbutton == 'savemenu') {
				if ( form.menutype.value == '' ) {
					alert( 'Введите имя меню' );
					form.menutype.focus();
					return;
				}
				var r = new RegExp("[\']", "i");
				if ( r.exec(form.menutype.value) ) {
					alert( 'Имя меню не может содержать знак \'' );
					form.menutype.focus();
					return;
				}
				<?php
				if ( $new ) {
					?>
					if ( form.title.value == '' ) {
						alert( 'Введите имя модуля для вашего меню' );
						form.title.focus();
						return;
					}
					<?php
		...
		<tr height="45px;">
			...
			<?php
			$tip = 'Идентификационное имя для Joomla! для связи меню с кодом меню - должно быть уникальным. Настятельно рекомендуется давать имена БЕЗ ПРОБЕЛОВ!';
			echo mosToolTip( $tip );
			?>
			</td>
		</tr>
		...
		if ( $new ) {
			...
			<tr>
				...
				<?php
				$tip = 'Заголовок для mod_mainmenu модуля требуется для отображения этого меню';
				echo mosToolTip( $tip );
				?>
				<br/><br/><br/>
				<strong>
				* Новый модуль mod_mainmenu с заданным заголовком будет автоматически создан после сохранения меню. *
				<br/><br/>
				Параметры этого модуля могут быть потом отредактированы в 'Модули [Сайта]': Модули -> Модули сайта
				</strong>
				</td>
			</tr>
			<?php
		...
		function submitbutton(pressbutton) {
			if (pressbutton == 'copymenu') {
				if ( document.adminForm.menu_name.value == '' ) {
					alert( 'Введите имя для копии меню' );
					return;
				} else if ( document.adminForm.module_name.value == '' ) {
					alert( 'Введите имя для нового модуля' );
					return;
				} else {
					submitform( 'copymenu' );
				}
