...
function saveMenu() {
	...
	if ( $old_menutype == 'mainmenu' ) {
		if ( $menutype != 'mainmenu' ) {
			echo "<script> alert('Вы не можете переименовать \'mainmenu\' Это меню требуется для нормальной работы Joomla'); window.history.go(-1); </script>\n";
			exit;
		}
	}
	...
	if (strstr($menutype, '\'')) {
		echo "<script> alert('Имя меню не может содержать знак \''); window.history.go(-1); </script>\n";
		exit;
	}
	...
	foreach ( $menus as $menu ) {
		...
		if ( $params->menutype == $menutype ) {
			echo "<script> alert('Меню с таким именем уже существует - введите уникальное имя меню'); window.history.go(-1); </script>\n";
			exit;
		}
	}
...
function deleteConfirm( $option, $type ) {
	...
	if ( $type == 'mainmenu' ) {
		echo "<script> alert('Вы не можете удалить \'mainmenu\' поскольку это меню ядра'); window.history.go(-1); </script>\n";
		exit();
	}
...
function copyMenu( $option, $cid, $type ) {
	...
	$msg = 'Копия меню `'. $type .'` создана, включая - '. $total .' - пунктов';
	mosRedirect( 'index2.php?option=' . $option, $msg );
}
