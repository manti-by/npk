	...
	function editCategory( &$menu, &$lists, &$params, $option ) {
		...
		function submitbutton(pressbutton) {
			...
			if ( !$menu->id ) {
				...
				?>
				if ( form.name.value == '' ) {
					alert( 'Меню должно иметь заголовок' );
				} else {
					submitform( pressbutton );
				}
				<?php
		...
		<tr>
			<th>
			<?php echo $menu->id ? 'Изменить' : 'Добавить';?> пункт меню :: Блог - материалы категории архива
			</th>
		</tr>
		</table>
		...
		<tr valign="top">
			<td width="60%">
				...
				<tr>
					<td width="10%" align="right" valign="top">Имя:</td>
					<td width="200px">
					...
					<?php
					if ( !$menu->id ) {
						echo mosToolTip( 'Если оставить пустым, будет автоматически использоваться имя категории' );
					}
					?>
					</td>
				</tr>
				...
				<tr>
					<td valign="top" align="right">Публикация:</td>
					<td>
