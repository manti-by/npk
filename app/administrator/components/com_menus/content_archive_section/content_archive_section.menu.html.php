	...
	function editSection( &$menu, &$lists, &$params, $option ) {
		...
		function submitbutton(pressbutton) {
			...
			if ( !$menu->id ) {
				...
				?>
				if ( form.name.value == '' ) {
					alert( 'Пункт меню должен иметь заголовок' );
				} else {
					submitform( pressbutton );
				}
				<?php
		...
		<tr>
			<th>
			<?php echo $menu->id ? 'Изменить' : 'Добавить';?> Пункт меню :: Блог - материалы раздела архива
			</th>
		</tr>
		</table>
		...
		<tr valign="top">
			<td width="60%">
				...
				<tr>
					...
					<?php
					if ( !$menu->id ) {
						echo mosToolTip( 'Если оставить пустым, будет использовано имя раздела' );
					}
					?>
					</td>
				</tr>
