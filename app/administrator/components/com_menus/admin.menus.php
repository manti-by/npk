	...
	foreach ( $list as $mitem ) {
		...
		switch ( $mitem->type ) {
			...
			case 'newsfeed_link':
				...
				$list[$i]->descrip 	= 'Изменить эту ленту';
				$mitem->link .= '&Itemid='. $mitem->id;
				break;
...
function TrashMenuSection( $cid=NULL, $menutype='mainmenu' ) {
	...
	$msg = "Отправлено в корзину: ".$total .' объектов';	
...
function moveMenu( $option, $cid, $menutype ) {
	...
	if (!is_array( $cid ) || count( $cid ) < 1) {
		echo "<script> alert('Выберите объект для переноса'); window.history.go(-1);</script>\n";
		exit;
	}
...
function moveMenuSave( $option, $cid, $menu, $menutype ) {
	...
	$msg = count($cid) .' Пункты меню перенесены в '. $menu;
...
function copyMenuSave( $option, $cid, $menu, $menutype ) {
	...
	$msg = 'Скопировано '.count( $cid ).' пунктов меню в меню - ' . $menu;
	mosRedirect( 'index2.php?option='. $option .'&menutype='. $menutype .'&mosmsg='. $msg );
}
