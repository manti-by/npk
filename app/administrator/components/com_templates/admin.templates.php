...
function removeTemplate( $cid, $option, $client ) {
	...
	if ($cur_template == $cid) {
		mosErrorAlert( "Вы не можете удалить активный шаблон" );
	}
...
function editTemplateSource( $p_tname, $option, $client ) {
	...
	if ( $fp = fopen( $file, 'r' ) ) {
		...
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Операция неудачна: не могу открыть '. $file );
	}
}
...
function saveTemplateSource( $option, $client ) {
	...
	if ( !$template ) {
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Операция неудачна: не указан шаблон.' );
	}
	if ( !$filecontent ) {
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Операция неудачна: содержимое пусто.' );
	}
	...
	if ( is_writable( $file ) == false ) {
		mosRedirect( 'index2.php?option='. $option , 'Операция неудачна: '. $file .' не доступен на запись.' );
	}
	...
	if ( $fp = fopen ($file, 'w' ) ) {
		...
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Операция неудачна: не могу открыть для записи.' );
	}
...
function saveTemplateCSS( $option, $client ) {
	...
	clearstatcache();
	if ( is_writable( $file ) == false ) {
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Операция неудачна: Файл недоступен на запись.' );
	}
	...
	if ($fp = fopen ($file, 'w')) {
		...
		if ($enable_write) @chmod($file, $oldperms);
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Операция неудачна: Не могу открыть файл на запись.' );
	}
...
function savePositions( $option ) {
	...
	mosRedirect( 'index2.php?option='. $option .'&task=positions', 'Расположение сохранено' );
}
?>