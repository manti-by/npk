...
function copyModule( $option, $uid, $client ) {
	...
	$msg = 'Сделана копия модуля с именем ['. $row->title .']';
...
function saveModule( $option, $client, $task ) {
	...
	switch ( $task ) {
		case 'apply':
			$msg = 'Изменения в модуле: -'. $row->title.'- успешно сохранены';
...
function removeModule( &$cid, $option, $client ) {
	...
	if (count( $cid ) < 1) {
		echo "<script> alert('Выберите модуль для удаления'); window.history.go(-1);</script>\n";
		exit;
	}
	...
	foreach ($rows as $row) {
		...
		if ( $row->module == 'mod_mainmenu' ) {
			if ( strstr( $row->params, 'mainmenu' ) ) {
				echo "<script> alert('Вы не можете удалить модуль mod_mainmenu (mainmenu) это модуль ядра'); window.history.go(-1); </script>\n";
				exit;
			}
		}
	}
	...
	if (count( $err )) {
		...
		echo "<script>alert('Модуль(и): \'$cids\' не может быть удален, а только деинсталлирован как Joomla! модуль.');</script>\n";
	}
