...
class HTML_modules {
	...
	*/
	function showModules( &$rows, $myid, $client, &$pageNav, $option, &$lists, $search ) {
		...
		<tr>
			...
			Менеджер модулей <small><small>[ <?php echo $client == 'admin' ? 'Админка' : 'Сайт';?> ]</small></small>
			</th>
			<td width="right">
		...
		for ($i=0, $n=count( $rows ); $i < $n; $i++) {
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				<td align="left">
				<?php echo $row->module ? $row->module : "-Самописный-";?>
				</td>
			</tr>
			<?php
			$k = 1 - $k;
	...
	*/
	function editModule( &$row, &$orders2, &$lists, &$params, $option ) {
		...
		<tr>
			<th class="modules">
			<?php echo $lists['client_id'] ? 'Админка' : 'Сайт';?>
			Модуль:
			<small>
