...
class HTML_module {
	...
	function showInstalledModules( &$rows, $option, &$xmlfile, &$lists ) {
		if (count($rows)) {
			...
			<tr>
				<td colspan="3">
				Показаны только модули, которые могут быть удалены - некотрые модули, являются компонентами ядра и удалению не подлежат.
				<br /><br />
				</td>
			</tr>
			</table>
			...
			for ($i = 0, $n = count( $rows ); $i < $n; $i++) {
				...
				<tr class="<?php echo "row$rc"; ?>">
					...
					<td>
					<?php echo $row->client_id == "0" ? 'Сайт' : 'Админка'; ?>
					</td>
					<td>
			...
			?>
			<td class="small">
			Нет установленных пользовательских модулей
			</td>
			<?php
		}
		?>
