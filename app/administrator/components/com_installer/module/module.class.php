...
class mosInstallerModule extends mosInstaller {
	...
	*/
	function install( $p_fromdir = null ) {
		...
		if ($this->parseFiles( 'files', 'module', 'Нет файла отмеченного как файл модуля' ) === false) {
			return false;
		}
	...
	*/
	function uninstall( $id, $option, $client=0 ) {
		...
		if ($row->iscore) {
			HTML_installer::showInstallMessage( $row->title .' модуль ядра и не может быть удален.<br />Если он вам не нужен, снимите его с публикации', 'Удаление - ошибка', $this->returnTo( $option, 'module', $row->client_id ? '' : 'admin' ) );
			exit();
		}
