...
HTML_installer::showInstallForm( 'Установить новый модуль', $option, 'module', '', dirname(__FILE__) );
?>
<table class="content">
<?php
writableCell( 'media' );
...
function showInstalledModules( $_option ) {
	...
	$select[] 		= mosHTML::makeOption( '1', 'Модули админки' );
	...
	for ($i = 0; $i < $n; $i++) {
		...
		if (file_exists( $xmlfile )) {
			...
			$row->creationdate 	= normal ($row->creationdate );

			$element 			= &$root->getElementsByPath( 'author', 1 );
			$row->author 		= $element ? $element->getText() : '';
			$row->author 	= normal ($row->author);

			$element 			= &$root->getElementsByPath( 'copyright', 1 );
			$row->copyright 	= $element ? $element->getText() : '';
			$row->copyright 	= normal ($row->copyright);

			$element 			= &$root->getElementsByPath( 'authorEmail', 1 );
			$row->authorEmail 	= $element ? $element->getText() : '';
			$row->authorEmail = normal ($row->authorEmail);

			$element 			= &$root->getElementsByPath( 'authorUrl', 1 );
			$row->authorUrl 	= $element ? $element->getText() : '';
			$row->authorUrl = normal ($row->authorUrl);

			$element 			= &$root->getElementsByPath( 'version', 1 );
			$row->version 		= $element ? $element->getText() : '';
			$row->version 		= normal ($row->version);
		}
	}
