	...
	switch ($task) {
		...
		default:
			...
			if (file_exists( $path )) {
				...
				echo "Для элемента - [$element] - инсталлятор не найден";
			}
			break;
	}
} else {
	echo "Инсталлятор для элемента - [$element] - недоступен";
}
...
function uploadPackage( $installerClass, $option, $element, $client ) {
	...
	if (!(bool)ini_get('file_uploads')) {
		HTML_installer::showInstallMessage( "Инсталлятор не может работать при запрещенной загрузке файлов. Используйте метод установки их каталога.",
			'Инсталлятор - ошибка', $installer->returnTo( $option, $element, $client ) );
		exit();
	}

	// Check that the zlib is available
	if(!extension_loaded('zlib')) {
		HTML_installer::showInstallMessage( "Инсталлятор не может работать до установки и разрешения GZIP",
			'Инсталлятор - ошибка', $installer->returnTo( $option, $element, $client ) );
		exit();
	}
	...
	if (!$userfile) {
		HTML_installer::showInstallMessage( 'Не выбран файл', 'Загрузка нового модуля - ошибка',
	...
	if ($resultdir !== false) {
		if (!$installer->upload( $userfile['name'] )) {
			HTML_installer::showInstallMessage( $installer->getError(), 'Загрузка '.$element.' - Загрузка сорвана',
				$installer->returnTo( $option, $element, $client ) );
		}
		$ret = $installer->install();

		HTML_installer::showInstallMessage( $installer->getError(), 'Загрузка '.$element.' - '.($ret ? 'Успешная' : 'Сорвана'),
			$installer->returnTo( $option, $element, $client ) );
...
function installFromDirectory( $installerClass, $option, $element, $client ) {
	...
	if (!$userfile) {
		mosRedirect( "index2.php?option=$option&element=module", "Выберите каталог" );
	}
	...
	HTML_installer::showInstallMessage( $installer->getError(), 'Загрузка '.$element.' - '.($ret ? 'Успешная' : 'Ошибка'), $installer->returnTo( $option, $element, $client ) );
}
/**
*
* @param
...
function removeElement( $installerClass, $option, $element, $client ) {
	...
	mosRedirect( $installer->returnTo( $option, $element, $client ), $result ? 'Успешно ' . $msg : 'Неуспешно ' . $msg );
}
/**
		...
		if (is_writable( $baseDir )) {
			if (move_uploaded_file( $filename, $baseDir . $userfile_name )) {
				if (mosChmod( $baseDir . $userfile_name )) {
					...
					$msg = 'Не могу сменить права на загруженный файл.';
				}
			} else {
				$msg = 'Не могу переместить загруженный файл в каталог <code>/media</code>';
			}
		} else {
			$msg = 'Загрузка сорвана, каталог <code>/media</code> не доступен на запись.';
		}
	} else {
		$msg = 'Загрузка сорвана, каталог <code>/media</code> не существует.';
	}
	return false;
}
?>