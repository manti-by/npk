	...
	*/
	function install( $p_fromdir = null ) {
		...
		if (!file_exists( $this->elementDir() ) && !mosMakePath( $this->elementDir() )) {
			$this->setError(1, 'Ошибка создания каталога "' . $this->elementDir() . '"' );
			return false;
		}
	...
	function uninstall( $id, $option, $client=0 ) {
		...
		if (trim( $id )) {
			if (is_dir( $path )) {
				...
				HTML_installer::showInstallMessage( 'Папка не существует, немогу удалить файлы', 'Деинсталляция - ошибка',
					$this->returnTo( $option, 'template', $client ) );
			}
		} else {
			HTML_installer::showInstallMessage( 'ID шаблона пустой, не могу удалить файлы', 'Деинсталляция - ошибка',
				$this->returnTo( $option, 'template', $client ) );
			exit();
		}
	}
