...
class mosInstallerComponent extends mosInstaller {
	...
	*/
	function install($p_fromdir = null) {
		...
		if (file_exists($this->elementDir())) {
			$this->setError( 1, 'Другой компонент уже занимает папку: "' . $this->elementDir() . '"' );
			return false;
		}

		if(!file_exists($this->elementDir()) && !mosMakePath($this->elementDir())) {
			$this->setError( 1, 'Не могу создать папку "' . $this->elementDir() . '"' );
			return false;
		}

		if(!file_exists($this->componentAdminDir()) && !mosMakePath($this->componentAdminDir())) {
			$this->setError( 1, 'Не могу создать папку "' . $this->componentAdminDir() . '"' );
			return false;
		}
		...
		if (!is_null($installfile_elemet)) {
			...
			if (!file_exists($this->componentAdminDir().$installfile_elemet->getText())) {
				if(!$this->copyFiles($this->installDir(), $this->componentAdminDir(), array($installfile_elemet->getText())))  			{
					$this->setError( 1, 'Не могу скопировать PHP установочный файл.' );
					return false;
				}
			}
		...
		if(!is_null($uninstallfile_elemet)) {
			if (!file_exists($this->componentAdminDir().$uninstallfile_elemet->getText())) {
				if(!$this->copyFiles($this->installDir(), $this->componentAdminDir(), array($uninstallfile_elemet->getText()))) {
					$this->setError( 1, 'Не могу скопировать PHP деинсталляционный файл' );
					return false;
				}
			}
		}
	...
	*/
	function uninstall( $cid, $option, $client=0 ) {
		...
		if ($row->iscore) {
			HTML_installer::showInstallMessage("Компонент $row->name - копмонент ядра, и не можут быть удален.<br />Вы можете его снять с публикации если он вам не нужен", 'Удаление - ошибка',
				$this->returnTo( $option, 'component', $client ) );
			exit();
		}
		...
		if (count($filesindir) > 0) {
			...
			if(!$found) {
				HTML_installer::showInstallMessage('Неверный XML файл','Ошибка деинсталляции',
					$this->returnTo( $option, 'component', $client ) );
				exit();
			}
		} else {
		...
		if (trim( $row->option )) {
			...
			HTML_installer::showInstallMessage( 'Файл опций пуст, не могу удалить файлы', 'Ошибка деинсталляции', $option,'component');
			exit();
		}
