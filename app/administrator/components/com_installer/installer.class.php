...
class mosInstaller {
	...
	function extractArchive() {
		...
		if (eregi( '.zip$', $archivename )) {
			...
			if (!$archive->extractModify( $extractdir, '' )) {
				$this->setError( 1, 'Ошибка извлечения' );
				return false;
			}
		}
	...
	*/
	function findInstallFile() {
		...
		if (count( $files ) > 0) {
			...
			$this->setError( 1, 'ОШИБКА: Не могу найти Joomla! XML установочный файл в пакете.' );
			return false;
		} else {
			$this->setError( 1, 'ОШИБКА: Не могу найти XML установочный файл в пакете.' );
			return false;
		}
	}
	/**
	...
	*/
	function readInstallFile() {
		...
		if ($this->installFilename() == "") {
			$this->setError( 1, 'Не задано имя файла' );
			return false;
		}
		...
		if ($root->getTagName() != 'mosinstall') {
			$this->setError( 1, 'Файл :"' . $this->installFilename() . '" не является допустимым инсталляционным файлом для Joomla!' );
			return false;
		}
	...
	*/
	function install() {
		die( 'Метод "install" не может быть вызват классом ' . strtolower(get_class( $this )) );
	}
	/**
	* Abstract uninstall method
	*/
	function uninstall() {
		die( 'Метод "uninstall" не может быть вызват классом ' . strtolower(get_class( $this )) );
	}
	/**
	* return to method
	*/
	...
	*/
	function preInstallCheck( $p_fromdir, $type ) {
		...
		if ($this->installType() != $type) {
			$this->setError( 1, 'XML установочный файл не для "'.$type.'".' );
			return false;
		}
			...
			if (basename( $file->getText() ) != $file->getText()) {
				...
				if ($adminFiles){
					if (!mosMakePath( $this->componentAdminDir(), $newdir )) {
						$this->setError( 1, 'Не могу создать каталог "' . ($this->componentAdminDir()) . $newdir . '"' );
						return false;
					}
				} else {
					if (!mosMakePath( $this->elementDir(), $newdir )) {
						$this->setError( 1, 'Не могу создать каталог "' . ($this->elementDir()) . $newdir . '"' );
						return false;
					}
				}
			}
		...
		if (is_array( $p_files ) && count( $p_files ) > 0) {
			foreach($p_files as $_file) {
				...
				if (!file_exists( $filesource )) {
					...
					$this->setError( 1, "Файл $filedest уже существует - Вы пытаетесь установить его дважды?" );
					return false;
				} else {
					...
					if( !( copy($filesource,$filedest) && mosChmod($filedest) ) ) {
						$this->setError( 1, "Не могу скопировать файл: $filesource как $filedest" );
						return false;
					}
				}
			}
