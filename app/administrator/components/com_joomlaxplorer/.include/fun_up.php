...
function upload_items($dir) {		// upload file
	...
	if(isset($GLOBALS['__POST']["confirm"]) && $GLOBALS['__POST']["confirm"]=="true") {	
		...
		for($i=0;$i<$cnt;$i++) {
			...
			// Upload
			$ok = @jx_File::move_uploaded_file($tmp, $abs);
	...
	echo "<br /><form enctype=\"multipart/form-data\" action=\"".make_link("upload",$dir,NULL). "\" method=\"post\">
		...
		<table style=\"width:60%;\" border=\"1\" class=\"adminform\">
			<tr><td class=\"quote\" colspan=\"2\">Максимальный размер файла = <strong>". ((get_max_file_size() / 1024) / 1024)." MB</strong><br />
				Максимальный лимит загрузки = <strong>". ((get_max_upload_limit() / 1024) / 1024)." MB</strong>
			</td></tr>
			";
	for($i=0;$i<10;$i++) {
		$class = $i % 2 ? 'row0' : 'row1';
