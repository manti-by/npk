...
//-->
	...
	// ALL OK?
	if(!@jx_File::file_exists(get_abs_dir($new_dir))) show_error(get_abs_dir($new_dir).": ".$GLOBALS["error_msg"]["targetexist"]);
	...
	for($i=0;$i<$cnt;++$i) {
		...
		if(!@jx_File::file_exists($abs_item)) {
		...
		if(@jx_File::file_exists($abs_new_item)) {
		...
		if($GLOBALS["action"]=="copy") {
			if(@is_link($abs_item) || get_is_file($abs_item)) {
				...
				$ok=@jx_File::copy( $abs_item ,$abs_new_item); //||@file_exists($abs_new_item);
			...
			elseif(@get_is_dir($abs_item)) {
				...
				$ok=jx_File::copy_dir( $dir, $abs_new_item);
			}
		}
		else {
			$ok= jx_File::rename($abs_item,$abs_new_item);
		}
