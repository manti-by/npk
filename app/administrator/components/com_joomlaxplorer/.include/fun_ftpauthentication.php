...
function ftp_authentication( $ftp_login='', $ftp_pass='') {
	...
	if( $ftp_login != '' || $ftp_pass != '' ) {
		...
		if( empty( $url )) {			
			echo jx_alertBox( 'Не могу распознать имя хоста. Пожалуйста, введите адрес в формате: hostname:21' );
			echo jx_scriptTag('', '$(\'loadingindicator\').innerHTML = \'\';' );
			echo 'Не могу распознать имя хоста. Пожалуйста, введите адрес в формате: hostname:21';
			exit;
		}
		...
		<?php
	show_header('Ввод данных для FTP авторизации');
	?><br/>
