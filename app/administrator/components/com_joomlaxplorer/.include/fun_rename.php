...
function rename_item($dir, $item) {		// rename directory or file
	...
	if(isset($GLOBALS['__POST']["confirm"]) && $GLOBALS['__POST']["confirm"]=="true") {
		...
		if(@jx_File::file_exists($abs_new)) {
			show_error($newitemname.": ".$GLOBALS["error_msg"]["itemdoesexist"]);
		}
		$perms_old = jx_File::fileperms( $abs_old );
		
		$ok=jx_File::rename( get_abs_item($dir,$item), get_abs_item($dir,$newitemname) );
		if( jx_isFTPMode()) {
			$abs_new = get_item_info($dir,$newitemname);
		}
		
		jx_File::chmod( $abs_new, $perms_old );
