...
{						// make table of files in dir
	...
	$handle = @jx_File::opendir(get_abs_dir($dir));
	
	if($handle===false && $dir=="") {
	  	$handle = @jx_File::opendir($homedir . $GLOBALS['separator']);
	}
	...
	// Read directory
	while(($new_item = @jx_File::readdir($handle))!==false) {
		...
		if(!@jx_File::file_exists($abs_new_item)) //show_error($dir."/$abs_new_item: ".$GLOBALS["error_msg"]["readdir"]);
		if(!get_show_item($dir, $new_item)) continue;
		
		$new_file_size = @jx_File::filesize($abs_new_item);
		...
		if(get_is_dir( $abs_new_item)) {
			...
			if($GLOBALS["order"]=="mod") {
				$dir_list[$new_item_name] =
					@jx_File::filemtime($abs_new_item);
...
function print_table($dir, $list, $allow) {	// print table of files
	...
	if( $dir != "" || strstr( $dir, _QUIXPLORER_PATH ) ) {
	  echo "<tr class=\"row1\">
	  			...
	  			<td valign=\"baseline\">
	  				...
	  				<img border=\"0\" width=\"22\" height=\"22\" align=\"absmiddle\" src=\""._QUIXPLORER_URL."/_img/_up.png\" alt=\"".$GLOBALS["messages"]["uplink"]."\" title=\"".$GLOBALS["messages"]["uplink"]."\"/>&nbsp;&nbsp;..</a>
	  			</td>
	  			<td>&nbsp;</td>
	  			<td>&nbsp;</td>
	  			<td>&nbsp;</td>
	...
	while(list($item,$info) = each($list)){
		...
		$is_writable = @jx_File::is_writable( $abs_item );
		$is_chmodable = @jx_File::is_chmodable( $abs_item );
		$is_readable =@jx_File::is_readable( $abs_item );
		$is_deletable = @jx_File::is_deletable( $abs_item );
		
		$target="";
		$extra="";
		if(@jx_File::is_link($abs_item)) $extra=" -> ".@readlink($abs_item);
	...
	// permissions
		...
		// VIEW
		if( jx_File::is_readable( $abs_item ) && get_is_file( $abs_item)) {
		...
		// DELETE
		if(get_is_file( $abs_item)) {
			if($allow && jx_File::is_deletable( $abs_item )) {
...
function list_dir($dir) {			// list directory contents
	...
	if($allow && @jx_File::is_writable( get_abs_dir( $dir)) ) {
		echo "<td align=\"right\">
	...
	if($GLOBALS["order"]=="name") echo $_img; echo '</a>';
	echo "</th>
