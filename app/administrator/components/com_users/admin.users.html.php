	...
	function edituser( &$row, &$contact, &$lists, $option, $uid, &$params ) {
		...
		function submitbutton(pressbutton) {
			...
			if (trim(form.name.value) == "") {
				...
				alert( "Логин содержит недопустимые символы или слишком короток." );
			} else if (trim(form.email.value) == "") {
				alert( "Введите e-mail." );
			} else if (form.gid.value == "") {
				alert( "Назначьте пользователю групу." );
			} else if (trim(form.password.value) != "" && form.password.value != form.password2.value){
				alert( "Пароль не равны." );
			} else if (form.gid.value == "29") {
				alert( "Введите другую группу, нельзя выбрать `Public Frontend`" );
			} else if (form.gid.value == "30") {
				alert( "Введите другую группу, нельзя выбрать `Public Backend`" );
			} else {
				submitform( pressbutton );
			}
		}
		...
		<tr>
			<th class="user">
			Пользователь: <small><?php echo $row->id ? 'Изменить' : 'Добавить';?></small>
			</th>
		</tr>
		</table>
		...
		<tr>
			<td width="60%" valign="top">
				...
				<tr>
					...
					<td>
					<input tittle="Максимум 50 символов" type="text" name="name" class="inputbox" size="40" value="<?php echo $row->name; ?>" maxlength="50" />
					</td>
				</tr>
				<tr>
					<td>
					...
					<td>
					<input tittle="Максимум 25 символов" type="text" name="username" class="inputbox" size="40" value="<?php echo $row->username; ?>" maxlength="25" />
					</td>
				<tr>
					<td>
					Email:
			...
			<td width="40%" valign="top">
				...
				if ( !$contact ) {
					...
					<tr>
						<td>
						<br />
						Нет связанной контактной информации для этого пользователя:
						<br />
						Смотрите 'Компоненты -> Контакты -> Контакты' для деталей.
						<br /><br />
						</td>
					</tr>
					</table>
					...
					<tr>
						<td colspan="2">
						<br /><br />
						<input class="button" type="button" value="Изменить данные Контакта" onclick="javascript: gotocontact( '<?php echo $contact[0]->id; ?>' )">
						<i>
						<br />
						'Компоненты -> Контакты -> Контакты'.
						</i>
						</td>
					</tr>
					</table>
