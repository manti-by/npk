...
function saveUser( $task ) {
	...
	if ($isNew) {
		...
		if ( $row->gid != $original->gid ) {		
			if ( $original->gid == 25 ) {					
				...
				echo "<script> alert('Вы не можете изменить его группу пользователей, поскольку это - единственный активный Супер-Администратор сайта'); window.history.go(-1); </script>\n";
					exit();
				}
			}
			...
			if (( $user_group == 'super administrator' && $my->gid != 25) ) {
				...
				echo "<script> alert('Вы не можете изменить эту группу, поскольку вы не Super Administrator этого сайта'); window.history.go(-1); </script>\n";
				exit();
			} else if ( $my->gid == 24 && $original->gid == 24 ) {
				// disallow change of super-Admin by non-super admin
				echo "<script> alert('Вы не можете изменить группу другого Администратора, поскольку вы не Super Administrator этого сайта'); window.history.go(-1); </script>\n";
				exit();
	...
	*/
	...
	if (!in_array($row->gid,getGIDSChildren($my->gid))) {
		// disallow creation of Super Admin by non Super Admin users
		echo "<script> alert('Вы не можете создать пользователя этой группы, только Super Administrators имеет на это право.'); window.history.go(-1); </script>\n";
		exit();
	}
	...
	switch ( $task ) {
		case 'apply':
			$msg = 'Успешно записаны изменения для пользователя: '. $row->name;
			mosRedirect( 'index2.php?option=com_users&task=editA&hidemainmenu=1&id='. $row->id, $msg );
			break;

		case 'save':
		default:
			$msg = 'Успешно записан пользователь: '. $row->name;
	...
	if ( !$msg && count( $cid ) ) {
		...
		foreach ($cid as $id) {
			...
			// cannot delete Super Admin where it is the only one that exists
				$msg = "Вы не можете удалить этого Супер-Администратора, поскольку это едиственный Супер-Администратор этого сайта";
			} else {
				// delete user
				$obj->delete( $id );
				$msg = $obj->getError();
...
function logoutUser( $cid=null, $option, $task ) {
	...
	josSpoofCheck(null, null, 'request');
 	...
 	if ( is_array( $cid ) ) {
		if (count( $cid ) < 1) {
			mosRedirect( 'index2.php?option='. $option, 'Выберите пользователя' );
		}
		...
		// check to see whether a Administrator is attempting to log out a Super Admin
		if ( $my->gid == 24 && $temp->gid == 25 ) {
			echo "<script> alert('Вы не можете отключить пользователя с правами Super Administrator'); window.history.go(-1); </script>\n";
			exit();
		}
		$ids = 'userid=' . (int) $cid;
	}
	...
	if (is_array( $cid ) && count( $cid )) {
		...
		foreach ($cid as $id) {
			...
			if ( !$allowActionToMyself && $id == $my->id ){
 				$msg .= 'Вы не можете '. $actionName .' в отношении самого себя!';
